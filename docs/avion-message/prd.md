...
*   **検索効率:** Phase 1（サーバーサイド暗号化）ではサーバーサイド全文検索を100ms以内で実行。Phase 2（E2E暗号化）ではクライアントサイドインデックスによるローカル検索を実現し、サーバー側で復号不可なメッセージのプライバシーを保護。会話のコンテキスト検索により関連メッセージを効率的に発見。
...
*   添付ファイル対応
    ...
    - ドキュメントファイルの共有
    - ファイルサイズ制限（100MB）
    - サムネイル自動生成（avion-mediaに委譲）
    - ウイルススキャン連携（avion-mediaに委譲）
*   メッセージの編集・削除
    - 送信後15分以内の編集
    - メッセージ削除（自分のみ/全員）
    - 編集履歴の表示
...
*   管理者機能
    ...
    - 不適切コンテンツの一括削除
    - ユーザー報告への対応機能
    - メッセージ内容の検査（Phase 1: サーバーサイド暗号化時はフルアクセス可能。Phase 2: E2E暗号化有効時はメタデータのみ）
    - 違反ユーザーのメッセージ履歴調査（E2E暗号化時はメタデータ調査のみ）
    - ハイブリッド通報対応（Signal方式: ユーザー端末から復号済みメッセージが添付される仕組み）
*   一括操作機能
    - メッセージの一括選択・削除
    - 会話の一括アーカイブ
    - 未読メッセージの一括既読
...
*   **AIによる内容分析:** E2E暗号化有効時はサーバー側でのメッセージ内容分析は不可能。サーバーサイド暗号化モード（Phase 1）ではメタデータ分析のみ許可。ユーザー通報時はSignal方式（クライアントから復号済みメッセージを添付）で対応。

## サービス間責務境界

avion-messageと他サービスの責務境界を以下に明確化する。

### 通知（avion-notification との境界）

- **avion-message の責務**: メッセージ固有の通知設定管理（会話単位のミュート、Do Not Disturb、メンション通知の優先度設定）。通知要否の判定ロジックを保持する。
- **avion-notification の責務**: 通知の実際の生成・配信（プッシュ通知、メール、SSE）。avion-messageはメッセージ送信・配信・既読等のイベントをNATS JetStream経由で発行し、avion-notificationがそれを購読して通知を生成・配信する。
- **境界ルール**: avion-messageは通知内容の生成や配信処理を直接行わない。イベント発行による委譲のみ。

> **通知設定の優先度解決**: avion-notification のグローバル設定が最優先される。本サービスの会話単位設定は、グローバル設定がONの場合のみ有効。詳細は [avion-notification PRD - 通知設定の優先度解決ルール](../avion-notification/prd.md) を参照。

### メディア（avion-media との境界）

- **avion-message の責務**: 添付ファイルのメタデータ管理（ファイルID、ファイル名、サイズ、MIME type、サムネイルURL）。メッセージとファイルの紐付け。
- **avion-media の責務**: ファイルのアップロード処理、ストレージ管理、サムネイル自動生成、ウイルススキャン連携。
- **境界ルール**: avion-messageはファイルの実体を直接操作しない。avion-mediaが返すファイルIDとメタデータを参照するのみ。

### 暗号化鍵（avion-auth との境界）

- **avion-auth の責務**: JWT認証トークン、セッション管理、ユーザー認証に関する鍵管理。
- **avion-message の責務**: Signal ProtocolによるE2E暗号化鍵の独立管理。X3DH鍵交換、Double Ratchetアルゴリズム、デバイス固有の暗号化鍵ペア、Sender Key配布。
- **境界ルール**: JWT/セッション鍵はavion-authの管轄。E2E暗号化鍵（Identity Key、Signed Pre Key、One-Time Pre Keys、Sender Key）はavion-messageが独立して管理する。両者の鍵は用途が異なり、相互に依存しない。

### 管理機能（avion-system-admin との境界）

- **avion-message の責務**: メッセージ固有の管理操作（メッセージの強制削除、メッセージメタデータ調査、ハイブリッド通報対応、コンプライアンス監査ログの記録）。
- **avion-system-admin の責務**: システム横断的な監査・管理（ユーザーアカウントの停止・凍結、プラットフォーム全体の統計・レポート、横断的なコンプライアンス管理）。
- **境界ルール**: avion-messageはメッセージドメインに閉じた管理操作のみ実行する。管理者アクション実行時は `ComplianceActionTakenEvent` をavion-system-adminに発行し、システム横断的な監査はavion-system-adminに委譲する。

### 検索（avion-search との境界）

- **avion-message の責務**: Phase 1（サーバーサイド暗号化）ではメッセージの全文検索機能をサービス内で提供。Phase 2（E2E暗号化）では、E2E暗号化メッセージの検索はクライアントサイドインデックスにより実行（サーバー側で復号不可のため）。
- **avion-search の責務**: 非暗号化メタデータ（送信者、日時、会話名、添付ファイルの種類等）の検索インデックス管理。
- **境界ルール**: E2E暗号化メッセージの本文検索はサーバー側では不可能であり、クライアント側で実行する。avion-searchにはメッセージ本文を送信しない。非暗号化メタデータの検索のみavion-searchに委譲する。

### ユーザー情報（avion-user との境界）

- **avion-message の責務**: 会話参加者のIDと権限情報の管理。ブロック状態の参照（avion-userから取得）。
- **avion-user の責務**: ユーザープロフィール、フォロー/ブロック関係の管理。
- **境界ルール**: avion-messageはユーザーの詳細情報を保持しない。ブロック状態の確認はavion-userへの問い合わせで行う。
...
- **MessageID**: メッセージの一意識別子（UUID v7、Backend採番、時系列ソート可能）
...
#### MessageNotificationSettingsService
**責務**: メッセージ固有の通知設定管理と通知要否判定（通知の実際の生成・配信はavion-notificationに委譲）
- **メソッド**:
  - `shouldNotify(message, recipient)`: メッセージの通知要否判定（ミュート設定、DND設定、メンション優先度を考慮）
  - `respectDoNotDisturb(userSettings)`: DND設定確認
  - `isConversationMuted(userID, conversationID)`: 会話のミュート状態確認
  - `getMentionPriority(message, userSettings)`: メンション通知の優先度取得
...
3. サーバーサイドで全文検索を実行（PostgreSQL全文検索）
4. 日付、送信者、ファイルタイプでフィルタリング
5. 関連度スコアでソート
6. ページネーション処理
7. SearchResultDTO を返却

**Phase 2（E2E暗号化有効時）:**
1. ユーザーは検索ボックスにキーワードを入力
2. クライアントサイドの検索インデックスを参照（サーバー側で復号不可のため）
3. 日付、送信者、ファイルタイプでフィルタリング
4. 非暗号化メタデータの検索はavion-searchに委譲
5. SearchResultDTO を返却
...
*   **メッセージキュー**: NATS JetStreamによる配信保証
...
*   **テスト要件**: ユニットテスト85%以上のコードカバレッジ、クリティカルパス95%以上、統合テスト自動化
...
管理者が法的要求やユーザー通報に基づいてメッセージ内容を調査可能な状態で、安全なメッセージングの基盤をリリースする。
...
*   メッセージリアクション、添付ファイル対応
*   サーバーサイド全文検索
*   管理者によるメッセージ内容調査（フルアクセス可能）
*   スパム対策・メッセージリクエスト機能

**暗号化モデル:**
*   通信路: TLS 1.3
*   保存時: PostgreSQL TDE + アプリケーション層AES-256-GCM
*   サーバーが暗号化鍵を管理するため、法的要求時にメッセージ内容を復号可能

### Phase 2: E2E暗号化のオプトイン

プライバシーを重視するユーザー向けに、会話単位でE2E暗号化をオプトインで提供する。

**含まれる機能:**
*   Signal Protocolベースのエンドツーエンド暗号化（会話単位でオプトイン）
    - 暗号化ライブラリ: libsignal (Rust) + CGo バインディング
    - 参考実装: gwillem/signal-go, Beeper Signal bridge (mautrix/signal) パターン
    - CGo依存: avion-mediaのbimg/libvipsと同様のパターン
    - ビルド要件: Rust toolchain 1.75+、CGO_ENABLED=1
*   X3DH鍵交換、Double Ratchetアルゴリズム
*   Forward Secrecy
*   マルチデバイス鍵同期（libsignal Sender Key Distribution Messageパターン）
    - デバイスごとに独立した鍵ペア
    - Sender Key: グループメッセージ用の対称鍵配布
    - 新デバイス追加時: 既存デバイスからの鍵転送フロー
*   暗号化ステータスの可視化
*   検索可能暗号化（クライアントサイドインデックス）

**E2E暗号化有効時の管理者調査:**
*   `InvestigateMessages` RPC はメタデータのみ返却（送信者、受信者、時刻、頻度、添付ファイルの有無・サイズ）
*   メッセージ本文へのアクセスは不可

**Phase 1→2 移行戦略:**
*   既存メッセージ（Phase 1）: AES-256-GCM at-rest暗号化のまま保持（再暗号化しない）
*   新規メッセージ（Phase 2以降）: E2E暗号化が有効な会話ではlibsignal E2Eで暗号化
*   暗号化バージョンフィールド（`encryption_version: 1 or 2`）でメッセージごとに暗号化方式を識別
*   クライアントは `encryption_version` に基づき適切な復号パスを選択

### WebSocket K8s Pod間接続状態共有

Kubernetes上での複数Pod運用時、WebSocket接続状態をPod間で共有し、メッセージを正しいPodにルーティングします。

*   **Redis Hash**: `ws:connections:{user_id}` にデバイスごとの接続先Pod IDをマッピング
*   **メッセージルーティング**: NATS JetStreamでPod間メッセージ転送。ローカルPodの場合は直接WebSocket配信、リモートPodの場合はNATS経由で転送
*   **Pod再起動時**: クライアントがclose frame (1012 Service Restart) を受信後、Exponential Backoffで自動再接続。新Podに接続マッピングを更新
*   **Sticky Session**: Kubernetes Serviceで `sessionAffinity: ClientIP`（タイムアウト3600秒）を使用し、同一クライアントからの再接続を同一Podにルーティング

### ハイブリッド通報フロー

E2E暗号化が有効な会話でも、ユーザー通報に対応するためにSignal方式のハイブリッドフローを採用する。

1. 被害ユーザーが問題のメッセージを選択して「通報」をタップ
2. クライアントアプリが復号済みメッセージテキスト、メタデータ、コンテキストメッセージ、通報理由を添付して通報を送信
3. サーバーは通報内容を受理し、モデレーションキューに追加
4. モデレーターが確認し、該当メッセージの強制削除や送信者へのペナルティを決定
5. 通報者へ結果通知

**注意:** サーバーはE2E暗号化メッセージを直接復号する能力を持たない。通報内容の真正性はメタデータとの照合により検証する。

## エラーハンドリング戦略

エラーハンドリングの詳細は [Design Doc: エラーハンドリング戦略](./designdoc.md#8-エラーハンドリング戦略) を参照。

### 概要

*   **エラーコードプレフィックス**: `MESSAGE`（`MESSAGE_DOMAIN_*`, `MESSAGE_USECASE_*`, `MESSAGE_INFRA_*`, `MESSAGE_HANDLER_*`）
*   **命名規則**: `[SERVICE]_[LAYER]_[ERROR_TYPE]`形式（[エラーコード標準化ガイドライン](../common/errors/error-standards.md)に準拠）
*   **gRPCステータスマッピング**: ドメインエラーからgRPCステータスコードへの標準的なマッピング
*   **PIIマスク方針**: メッセージ本文・暗号化コンテンツ・秘密鍵は絶対にログ出力しない。メタデータのマスクが中心
*   **ログレベル基準**:
    - Debug: 開発・デバッグ用詳細情報
    - Info: 正常な業務処理の記録
    - Warn: 異常だが自動回復可能な事象
    - Error: 処理失敗で手動対応が必要
    - CRITICAL: データ整合性の致命的破壊（panicレベル）
*   **エラー復旧パターン**:
    - WebSocket再接続: Exponential Backoff（最大10回、最大30秒間隔）
    - メッセージ再送: 指数バックオフ（最大5回）、失敗時DLQ移動
    - DLQ処理: 手動確認 or 定期バッチ再処理、送信者への失敗通知

## Operations & Monitoring

運用・監視の詳細は [Design Doc: Operations & Monitoring](./designdoc.md#9-operations--monitoring) を参照。

### 概要

*   **主要メトリクス (Prometheus形式)**:
    - WebSocket接続数（`ws_active_connections_gauge`）
    - メッセージ配信レイテンシ（`msg_delivery_duration_seconds`）
    - E2E暗号化鍵交換成功率（`e2e_key_exchange_total`）
    - スパム検出率（`spam_detected_total`）
    - DLQメッセージ数（`dlq_messages_gauge`）

*   **アラート閾値**:
    - メッセージ配信レイテンシ p99: > 500ms でWARN、> 2s でCRITICAL
    - WebSocket接続エラー率: > 1% でWARN、> 5% でCRITICAL
    - E2E鍵交換失敗率: > 2% でWARN、> 10% でCRITICAL
    - DLQメッセージ数: > 100 でWARN、> 1000 でCRITICAL

*   **インシデント対応**:
    - CRITICALアラート → 5分以内にオンコールエンジニア応答
    - WARNアラート → 営業時間内に対応
    - ポストモーテム: 24時間以内に作成

*   **手動回復手順**:
    - WebSocket大量切断時のrolling restart
    - DLQメッセージの再処理
    - 暗号化鍵の不整合修復
    - メッセージ配信状態の整合性修復