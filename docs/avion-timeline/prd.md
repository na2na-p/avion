...
* **パフォーマンス**: Redisキャッシング戦略により、タイムライン取得レスポンスタイムを平均50ms以下、p99 150ms以下に短縮（キャッシュヒット時）。タイムラインタイプ別キャッシュヒット率目標: HOME/SOCIAL: 95%、LOCAL: 90%、LIST: 90%、GLOBAL/HASHTAG: 70%、MEDIA: 80%。データベースアクセスを大幅削減。
...
* [avion-moderation PRD](../avion-moderation/prd.md) - コンテンツモデレーション・フィルタリング連携
...
* **快適な閲覧体験**: 高速レスポンス（キャッシュヒット時 p99 150ms以下）とスムーズなページネーション、直感的なフィルタリングによる優れたUX
...
* **ハイブリッドFan-out戦略**: フォロワー数別最適配信
  ...
  - Pull型: フォロワー数10,000人超（オンデマンドで取得）
* **リアルタイム更新**: 平均3秒以内での新規投稿反映
  - NATS JetStreamによる低遅延イベント配信
...
キャッシュ戦略の詳細については [Redisキャッシュ戦略](../common/infrastructure/redis-cache-strategy.md) を参照。

#### 統合・連携
* **マイクロサービス連携**: avion-drop（投稿データ）、avion-user（フォロー・ミュート・ブロック・プライバシー設定）、avion-search（ハッシュタグ）、avion-moderation（コンテンツモデレーション判定）
* **イベント駆動アーキテクチャ**: NATS JetStream による非同期イベント処理
...
avion-timelineにおけるフィルタリング処理は、以下のサービス間責務分担に基づいて実装する。

### プライバシーフィルタリング（avion-user連携）

タイムライン生成時、avion-timelineは **avion-userのgRPC API** を呼び出してプライバシー関連情報を取得し、フィルタを適用する。

- **ミュート設定**: avion-userの `GetMutedUsers` APIでミュート中のユーザーID一覧を取得し、`GetMuteKeywords` APIでミュートキーワード一覧を取得して、タイムラインエントリーから該当投稿を除外
- **ブロック設定**: avion-userの `GetBlockedUsers` APIでブロック中のユーザーID一覧を取得し、ブロックユーザーの投稿を除外
- **フォロー関係**: avion-userの `GetFollowers` / `GetFollowing` APIでフォロー方向別の関係を取得し、ホームタイムラインの構築やFOLLOWERS_ONLY投稿のアクセス判定に使用
- **プライバシー設定**: avion-userの `GetSettings` API（プライバシー関連フィールドを含む）でアカウントの公開/非公開設定を取得し、可視性制御に使用

これらの情報はavion-timeline側でLRUキャッシュ（TTL: 5分）に保持し、パフォーマンスを確保する。キャッシュ無効化はavion-userからのNATS JetStreamイベント（ミュート変更、ブロック変更等）をトリガーとする。

### モデレーションフィルタリング（avion-moderation連携）

コンテンツモデレーションに関するフィルタリングは **avion-moderationのgRPC API** に問い合わせて判定する。

- **コンテンツフィルタ**: avion-moderationの `CheckContentVisibility` APIで投稿の表示可否を判定し、モデレーション対象の投稿をタイムラインから除外
- **NSFW判定**: avion-moderationの判定結果に基づき、ユーザー設定に応じたNSFWコンテンツのフィルタリングを実行

モデレーション判定結果もキャッシュ（TTL: 10分）に保持し、モデレーション状態変更イベント受信時にキャッシュを無効化する。

### avion-timeline自身のフィルタリング責務

以下のフィルタリングはavion-timeline自身が担当する:

- **タイムラインタイプ別フィルタ**: HOME/LOCAL/GLOBAL等のタイプに応じた投稿収集ルールの適用
- **動的フィルタ**: メディアのみ（MEDIA_ONLY）、リモートのみ（REMOTE_ONLY）等のユーザー指定フィルタ
- **可視性フィルタ**: PUBLIC/UNLISTED/FOLLOWERS_ONLY/PRIVATEに基づくアクセス制御（フォロー関係情報はavion-userから取得）
- **重複排除**: ソーシャルタイムライン等でのエントリー重複排除
- **時系列ソート**: タイムスタンプベースのソートとページネーション
...
* **avion-moderation**: コンテンツモデレーション判定の参照
...
- **不変条件** (12個):
  1. ConnectionIDは一意でありUUID v7形式である
...
- **不変条件** (8個):
  ...
  4. LRU Eviction戦略に従う
  5. メモリ使用量は80%以下を維持
  6. キャッシュヒット率はタイムラインタイプ別に監視される（HOME/SOCIAL: 95%、LOCAL: 90%、LIST: 90%、GLOBAL/HASHTAG: 70%、MEDIA: 80%）
  7. 無効化は即座に反映される
...
- **属性**:
  - EntryID (エントリーの一意識別子、UUID v7)
  - DropID (UUID v7 - 投稿の一意識別子)
...
- **属性**:
  - MemberID (メンバーエントリーの一意識別子、UUID v7)
  - UserID (メンバーのユーザーID)
  - ListID (所属リストID)
  - AddedAt (リストに追加された日時)
...
- **ListID**: UUID v7形式のリスト識別子
- **ConnectionID**: SSE接続の一意識別子（UUID v7）
- **DropID**: UUID v7形式の投稿識別子
...
- **メソッド**:
  ...
  - `determineFanOutStrategy(followerCount)`: Fan-out戦略を決定（Push/Hybrid/Pull）
...
- **メソッド**:
  ...
  - `executePushFanout(dropEvent)`: Push型Fan-out実行（フォロワー < 1,000人）
  - `executeHybridFanout(dropEvent)`: Hybrid型Fan-out実行（1,000〜10,000人）
  - `executePullFanout(dropEvent)`: Pull型Fan-out実行（フォロワー > 10,000人）
...
1. avion-dropがDrop作成時に`extractHashtags()`でハッシュタグを抽出
2. `DropCreatedEvent`に`ExtractedHashtags`フィールドを含めてNATS JetStreamに発行
3. avion-searchがイベントを消費し、ハッシュタグインデックスを更新
4. avion-timelineのハッシュタグタイムライン取得時にavion-searchの`GetDropsByHashtag` APIを呼び出し
5. 取得結果をタイムラインフォーマットに変換してクライアントに返却
...
2. `avion-drop` がDropEvent Value ObjectをNATS JetStreamに発行。
...
5. FanoutStrategy（Domain Service）がフォロワー数に基づき配信方式を決定：
   - < 1,000フォロワー: Push型（即座に全フォロワーのタイムラインを更新）
   - 1,000〜10,000フォロワー: Hybrid型（アクティブユーザーにPush、その他はPull）
   - > 10,000フォロワー: Pull型（リクエスト時に動的生成）
...
* **Fan-out戦略**: フォロワー数に応じたハイブリッド配信（< 1,000: Push、1,000〜10,000: Hybrid、> 10,000: Pull）
...
* **TimelineEntry**: DropID（UUID v7）、Timestamp（UTC、ミリ秒精度）、AuthorID、Position
* **Timeline キャッシュ**: Redis Sorted Set、24時間 TTL、LRU Eviction
* **List データ**: PostgreSQL 17永続化、リスト名（1-100文字）、説明（0-500文字）
...
* **タイムライン取得 (キャッシュヒット)**: 平均 50ms 以下、p99 150ms 以下
...
* **Timeline Entry永続化**: 複合キー方式を採用する。`user:{id}:home` キーでSorted Set構成、score=UnixNano、member=dropIDとする。ZRANGEBYSCORE で高速ページネーションを実現する
* **SSE Connection スケーリング**: Pod単位分散+NATSブロードキャスト方式を採用する。各Podが担当接続を保持し、NATSで全Podにイベント配信する。HPAで自動スケールする
* **TimelineUpdateEvent フォーマット**: Proto3定義+バージョンフィールド方式を採用する。ConnectRPCと統一し、後方互換性を自動保証する
* **キャッシュ再構築戦略**: 差分更新優先+全量フォールバック方式を採用する。通常はイベントから差分再構築し、30分以上のギャップが発生した場合は全量再構築を行う
* **Timeline収集範囲**: デフォルトはフォローベースとする。インスタンス管理者の設定で変更可能とし、連合リレー利用時は既知インスタンス全体に切替を推奨する（設定時に警告表示）
* **トランザクション境界**: 結果整合性+補償方式を採用する。各操作はローカルトランザクションとし、失敗時はNATS再配信で結果整合性を担保する
* **リスト共有機能**: Phase 2で実装する。visibilityフィールドでprivate/public/followers_onlyを制御する
* **ハッシュタグキャッシュ**: イベント駆動+TTL方式を採用する。avion-searchからのtrending_updatedイベントで無効化し、ベースラインTTL=5分とする
* **ミュート責務分担**: avion-userが永続化を担当し、mute.created/deletedイベントを発行する。avion-timelineがRedisにキャッシュしてフィルタリングを実行する
* **パフォーマンス最適化**: k-wayマージ方式を採用する。HOME/LOCALの2つのSorted Setからカーソルベースでk-wayマージし、重複排除を行う
* **国際化対応**: UTC統一+クライアント側変換方式を採用する。サーバーは全てUTCで保存・ソートし、タイムゾーン変換はフロントエンドの責務とする
* **アルゴリズム拡張**: Phase 3以降で検討する。scoreフィールドを拡張可能に設計しておく（現時点では時系列のみ）
* **GDPR対応**: イベント駆動削除方式を採用する。user.profile.deletedイベントを購読し、全タイムラインタイプから対象Dropを削除する。Redisキャッシュもパージする。user-deletion-cascade.mdと整合させる
* **監査要件**: 更新操作のみ構造化ログで記録する。タイムライン追加/削除/並べ替え操作をログに記録し、読み取りはメトリクスのみで監視する
* **マイグレーション戦略**: デュアルライト+段階移行方式を採用する。新旧両フォーマット並行稼働とし、新規書き込みは新フォーマット、読み取りは両方フォールバックとする。ダウンタイムなしで移行する
