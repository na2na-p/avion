...
*   **モデレーション階層構造（moderation優先）**: 本サービスの `auto_moderation_rules`（コミュニティローカルのキーワードフィルタ等）はコミュニティ内のみに適用される。`avion-moderation` のプラットフォーム全体ポリシーが常に上回り、コミュニティオーナーはプラットフォームポリシーを緩和できない。評価順序: (1) `avion-moderation` のプラットフォーム全体NGワード → ブロック (2) 本サービスのローカルルール → コミュニティ内のみ適用
