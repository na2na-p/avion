...
*   **コミュニティ管理:**
    ...
    - コミュニティ情報の編集・更新
    - コミュニティアバター・ヘッダー画像の設定（`avion-media` 連携: Presigned URL方式でアップロードし、MediaIDのみを保持）
    - コミュニティの削除・アーカイブ
    - コミュニティ検索・発見機能（全文検索は `avion-search` に委譲。本サービスはコミュニティメタデータを `avion-search` のインデックスに提供し、検索実行は `avion-search` の `SearchCommunities` APIを呼び出す）
    - 人気/推奨コミュニティの表示
...
*   **外部連携:**
    - ActivityPub対応（他のインスタンスのコミュニティとの連携）（Phase 3以降で実装）
    - 他サービス（avion-drop、avion-timeline）との連携
    - Webhooks（外部ボット、自動化ツールとの連携）（Phase 3以降で実装）
...
#### ModerationAppeal Aggregate
**責務**: モデレーション処理への異議申し立てを管理
- **集約ルート**: ModerationAppeal
- **不変条件**:
  - AppealReasonは必須（空文字不可）
  - ModerationLogIDは存在するモデレーションログを参照
  - AppealStatusは定義された値（pending, accepted, rejected）のいずれか
  - 同一ModerationLogIDに対する異議申し立ては1件のみ
  - レビュー済みの異議申し立ては再申請不可
- **ドメインロジック**:
  - `canAppeal(userID, membership)`: 異議申し立て可否の判定
  - `accept(reviewerID, comment)`: 異議申し立ての承認（制裁の取消）
  - `reject(reviewerID, comment)`: 異議申し立ての却下
  - `validate()`: 異議申し立て内容の妥当性検証
...
**所属**: Community Aggregate
**責務**: コミュニティ固有の自動モデレーション設定を管理
- **属性**:
  - RuleID（Entity識別子）
  - RuleName（ルール名）
  - RuleType（ルール種別：keyword, pattern, behavior）
  - RulePattern（パターン文字列）
  - Action（アクション：warn, delete, suspend）
  - IsActive（有効/無効）
  - CreatedByUserID（作成者）
  - CreatedAt（作成日時）
  - UpdatedAt（更新日時）
- **ビジネスルール**:
  - コミュニティローカルのルールのみ管理（グローバルルールは avion-moderation が管理）
  - プラットフォーム全体ポリシーを緩和するルールは設定不可

#### CommunityTemplate Entity
**所属**: Community Aggregate
**責務**: コミュニティ作成時のテンプレート設定を管理
- **属性**:
  - TemplateID（Entity識別子）
  - TemplateName（テンプレート名）
  - TemplateType（テンプレート種別：gaming, education, business等）
  - DefaultRules（デフォルトルール設定、JSON）
  - DefaultTopics（デフォルトトピック設定、JSON）
  - DefaultRoles（デフォルトロール設定、JSON）
  - IsPublic（公開/非公開）
  - CreatedByUserID（作成者）
  - CreatedAt（作成日時）
- **ビジネスルール**:
...
#### ModerationAppealRecord Entity
**所属**: ModerationAppeal Aggregate
**責務**: 異議申し立ての記録を管理
- **属性**:
  - RecordID（Entity識別子）
  - AppealerUserID（申請者）
  - ModerationLogID（対象モデレーションログ）
  - AppealReason（申請理由）
  - AppealStatus（状態：pending, accepted, rejected）
  - ReviewedByUserID（レビュー者）
...
**識別子関連**
- **CommunityID**: コミュニティの一意識別子（UUID v7）
- **MembershipID**: メンバーシップの一意識別子（UUID v7）
- **TopicID**: トピックの一意識別子（UUID v7）
- **InviteCode**: 招待コードの一意識別子（URL-safe文字列）
- **EventID**: イベントの一意識別子（UUID v7）
- **RuleID**: ルールの一意識別子（UUID v7）
...
3. コミュニティアバター・ヘッダー画像をアップロード（オプション）
   - フロントエンドは `avion-media` のPresigned URL APIを呼び出し、S3に直接アップロード
   - アップロード完了後、`avion-media` から返却されたMediaIDをコミュニティ作成リクエストに含める
   - 本サービスはMediaIDのみを保持し、メディアの実体管理は `avion-media` に委譲
...
8. アバター・ヘッダー画像のMediaIDが指定されている場合、`avion-media` の gRPC APIでMediaIDの存在確認と所有者検証を実施
9. CommunityFactory で Community Aggregate を生成（AvatarMediaID、HeaderMediaIDを含む）
10. Community Aggregate の validate() メソッドで全体の妥当性を確認
11. CommunityRepository 経由でデータベースに永続化
12. MembershipFactory でオーナーのメンバーシップを生成
13. MembershipRepository で永続化
14. CommunityEventPublisher で `community_created` イベントを発行
15. CreateCommunityResponse DTO を返却
...
5. `avion-search` の `SearchCommunities` gRPC APIを呼び出し、全文検索を委譲
6. `avion-search` から返却された検索結果（CommunityIDリスト）をもとに、本サービスのCommunityRepositoryからコミュニティ詳細情報を取得
7. 公開コミュニティのみをフィルタリング
8. ユーザーの興味関心に基づく推薦スコア算出（オプション）
9. CommunityListDTO を生成（各コミュニティの概要情報）
10. SearchCommunitiesResponse DTO を返却
11. フロントエンドは検索結果をカード形式で表示
...
1. 制裁を受けたメンバーが「異議申し立て」ボタンをクリック
2. 異議申し立て理由を入力するフォームが表示
3. フロントエンドは `avion-gateway` 経由で異議申し立てリクエストを送信
4. AppealModerationCommandUseCase がリクエストを処理
5. ModerationLogRepository から対象のモデレーションログを取得
6. 同一モデレーション処理への既存の異議申し立てがないかチェック
7. ModerationAppealFactory で ModerationAppeal Aggregate を生成
8. ModerationAppeal.validate() で妥当性検証
9. ModerationAppealRepository で永続化
10. CommunityEventPublisher で `appeal_created` イベントを発行
11. AppealModerationResponse DTO を返却
12. (非同期) コミュニティモデレーターに異議申し立て通知を送信
...
*   **モデレーション階層構造（moderation優先）**: 本サービスの `auto_moderation_rules`（コミュニティローカルのキーワードフィルタ等）はコミュニティ内のみに適用される。`avion-moderation` のプラットフォーム全体ポリシーが常に上回り、コミュニティオーナーはプラットフォームポリシーを緩和できない。評価順序: (1) 本サービスのローカルルール判定 → コミュニティ内のみ適用 (2) `avion-moderation` のプラットフォーム全体ポリシー判定（非同期） → グローバルルール違反は即座にコンテンツ除去（ローカルルール設定に関係なく強制）
*   **コミュニティ内投稿（Dropと同一モデル）**: Topic内の投稿は `avion-drop` のDropと同一モデルを使用する。本サービスはDropとTopic/Communityの関連付け（`community_id`, `topic_id`）のみを管理する。リアクション・ハッシュタグ・検索等の既存機能を全て再利用できる
*   **コミュニティフィード**: Topic別タイムラインは `avion-timeline` に委譲する。本サービスはTopic-Drop関連付けのデータを提供し、`avion-timeline` がフィード生成を担当する
