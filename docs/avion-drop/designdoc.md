...
- **キャッシュ/キュー:** Redis 8+
- **メッセージング:** NATS JetStream（サービス間イベント配信）
- **HTTPルーティング:** Chi v5
- **RPC:** ConnectRPC
- **ミドルウェア:** 標準net/httpベースの共通ミドルウェア
- **ORM:** GORM
- **テスト:** go.uber.org/mock/gomock, github.com/google/go-cmp/cmp, github.com/newmo-oss/ctxtime
...
- Dropの作成、取得、編集、論理削除を行うgRPC APIの実装。
...
- Drop作成、編集、削除、リアクション追加/削除時にイベントを発行 (NATS JetStream) し、他のサービス (Timeline, User, Search, ActivityPub, Notification) と連携する。特に `drop.created` / `drop.deleted` イベントは avion-user が消費し UserStats.DropCount を更新する。
...
- **ハッシュタグ/メンション解析の独自実装。** テキスト解析（抽出・正規化）ロジックはGo共有パッケージ（`pkg/textanalysis`）として切り出す。本サービスと `avion-search` が同一の共有パッケージを利用する。本サービスの `extractHashtags()` / `extractMentions()` は共有パッケージの呼び出しに置き換える。
...
- ScheduledDropStatus, DraftStatus, ReportReason, ReportStatus
- HashTag, Mention, URL
...
Domain Eventsは消費者の観点から以下の3カテゴリに分類される:

- **Core Events（外部消費）**: 他サービスが購読する主要イベント。スキーマ変更時は後方互換性を維持する必要がある
- **Internal Events（内部利用）**: avion-drop内部でのみ使用されるイベント
- **Future Events（将来実装）**: 将来の機能拡張で追加予定のイベント

※ 各イベントの詳細スキーマは [共通イベントスキーマ定義](../common/events/event-schemas.md) を参照

- DropCreatedEvent `[Core]`: コンテンツが公開され配信可能になった
  ```go
  type DropCreatedEvent struct {
      ...
      ExtractedHashtags []string    // 抽出済みハッシュタグ
      ExtractedMentions []UserID    // 抽出済みメンション
      MediaIDs          []MediaID   // 添付メディアID一覧
      ReplyToDropID     *DropID     // リプライ先DropID（nilの場合はルート投稿）
      SensitiveFlag     bool        // ユーザー自己申告のセンシティブフラグ
      OccurredAt        time.Time
  }
  ```
- DropEditedEvent `[Core]`: コンテンツが修正され信頼性が更新された
- DropDeletedEvent `[Core]`: コンテンツが利用不可になり関連データの整理が必要
- ReactionAddedEvent `[Core]`: エンゲージメントが発生し感情表現が記録された
- ReactionRemovedEvent `[Core]`: エンゲージメントが取り消され感情表現が撤回された
- BookmarkAddedEvent `[Core]`: Dropブックマーク追加イベント
- PollVotedEvent `[Future]`: 民主的意思決定への参加が記録された
- PollClosedEvent `[Future]`: 意思決定プロセスが完了し結果が確定した
- MediaAttachedEvent `[Internal]`: ビジュアルコンテンツが追加され表現が豊かになった
- DropLikedEvent `[Internal]`: Dropいいねイベント
- DropUnlikedEvent `[Internal]`: いいね取り消しイベント
- DropSharedEvent `[Internal]`: Drop共有イベント
- DropBookmarkedEvent `[Internal]`: Dropブックマークイベント
- DropUnbookmarkedEvent `[Internal]`: ブックマーク解除イベント
- DropRepostedEvent `[Core]`: Dropリポストイベント
- ReactionAggregatedEvent `[Internal]`: リアクション集計完了イベント
- ThreadCreatedEvent `[Internal]`: スレッド作成イベント
- ThreadUpdatedEvent `[Internal]`: スレッド更新イベント
- DropAddedToThreadEvent `[Internal]`: スレッドにDrop追加イベント
- ThreadReorderedEvent `[Internal]`: スレッド順序変更イベント
- ThreadSplitEvent `[Internal]`: スレッド分割イベント
- ThreadMergedEvent `[Internal]`: スレッド結合イベント
- ThreadClosedEvent `[Internal]`: スレッド閉鎖イベント
- ReplyCreatedEvent `[Core]`: 返信作成イベント
- ReplyDeletedEvent `[Core]`: 返信削除イベント
- QuoteDropCreatedEvent `[Core]`: 引用Drop作成イベント
- MentionAddedEvent `[Core]`: メンション追加イベント
- HashtagAddedEvent `[Core]`: ハッシュタグ追加イベント
- LinkAttachedEvent `[Internal]`: リンク添付イベント
- MediaReferencedEvent `[Internal]`: メディア参照イベント
- DraftSavedEvent `[Internal]`: 下書き保存イベント
- DraftDiscardedEvent `[Internal]`: 下書き破棄イベント
- DraftPublishedEvent `[Internal]`: 下書き公開イベント
- EditHistoryRecordedEvent `[Internal]`: 編集履歴記録イベント
- VersionCreatedEvent `[Future]`: バージョン作成イベント
- VersionRestoredEvent `[Future]`: バージョン復元イベント
- PollCreatedEvent `[Future]`: 投票作成イベント
- DropPinnedEvent `[Internal]`: Drop固定イベント
- DropUnpinnedEvent `[Internal]`: Drop固定解除イベント
- DropReportedEvent `[Core]`: Drop通報イベント
- ReportResolvedEvent `[Internal]`: 通報解決イベント
- DropImpressionRecordedEvent `[Internal]`: Drop閲覧記録イベント
- DropMarkedAsSensitiveEvent `[Internal]`: センシティブマーク付与イベント
- DropUnmarkedAsSensitiveEvent `[Internal]`: センシティブマーク解除イベント
...
- **Query Use Cases (参照系):**
  ...
  - ~~GetTrendingHashtagsQueryUseCase: トレンドハッシュタグ取得（GETリクエスト用）~~ ※廃止予定: avion-search の `GetTrendingHashtagsQueryUseCase` に委譲
...
- **External Service Implementations:**
  - NATSEventPublisher: NATS JetStreamでのイベント発行実装
...
- **Cache Components:**
  ...
  - ~~RedisHashtagTrendCache: トレンドハッシュタグキャッシュ~~ ※廃止予定: トレンド集計は avion-search に委譲
...
- **Background Jobs:**
  ...
  - ~~HashtagTrendAggregationJob: ハッシュタグトレンド集計ジョブ~~ ※廃止予定: トレンド集計は avion-search に委譲
...
- **Query Handlers (参照系):**
  ...
  - ~~GetTrendingHashtagsQueryHandler: トレンドハッシュタグ取得エンドポイント（GET）~~ ※廃止予定: Gateway経由で avion-search に委譲
  - GetDropEditHistoryQueryHandler: Drop編集履歴取得エンドポイント（GET）

### 6.2. 詳細ドメインモデル（DDD戦術パターン）
...
- **集約ルート**: Drop
- **不変条件**:
  - DropIDは変更不可（UUID v7、Backend採番）
  - 投稿テキストは最大500文字（Unicode準拠、絵文字対応）
  - 公開範囲（Visibility）の変更は作成者のみ可能
  - 削除されたDropは復元不可（論理削除、一定期間後にバッチで物理削除）
  - 編集は投稿から30分以内、最大5回まで
  - メディア添付は最大4つまで
...
- **ドメインロジック**:
  ...
  - ~~`toActivityPubNote()`: ActivityPub Note形式への変換~~ → `avion-activitypub` の `ActivityPubTranslator`（アンチコラプションレイヤー）に移譲。本サービスは `DropCreatedEvent` の発行のみを担う。
  - `applyContentWarning(warning)`: コンテンツ警告の適用
  - `schedulePublication(scheduledTime)`: 予約投稿設定（時刻妥当性検証）

> **コミュニティ内投稿について**: コミュニティへの投稿はDrop Aggregateと同一モデルで処理される。コミュニティとの関連付けはavion-communityサービスの `CommunityPost` Entityが管理するため、Drop Aggregateのドメインモデルにはコミュニティ関連のフィールドを含まない。詳細は [avion-community PRD](../avion-community/prd.md) を参照。
...
- **主要コンポーネント:**
    - `avion-drop (Go, Kubernetes Deployment)`: 本サービス。gRPCサーバー、NATS JetStreamイベント発行者。
    - `avion-gateway (Go)`: gRPCリクエストのルーティング元。
    - `avion-media (Go)`: メディア情報の関連付けで連携。
    - `PostgreSQL`: Dropデータおよびリアクションデータを永続化。
    - `Redis`: リアクション集計キャッシュ (Hash)、ユーザー別リアクションキャッシュ (Set)。
    - `NATS JetStream`: イベント配信（サービス間非同期通信）。
    - `Observability Stack`: メトリクス、トレース、ログ収集。
    - イベント購読者 (`avion-timeline`, `avion-user`, `avion-search`, `avion-activitypub`, `avion-notification`)。
- **構成図:** (アーキテクチャ概要図を参照)
    - [Avion アーキテクチャ概要](../common/architecture.md)
- **ポイント:**
    - Dropの作成、取得、論理削除とデータ永続化を行う。
...
- **フロー 1: Drop作成 (Command)**
    ...
    5. CreateDropCommandUseCase: EventPublisherを通じて `avion.drop.drop.created` イベントを発行
    6. CreateDropCommandHandler → Gateway: `CreateDropResponse { drop_id: "..." }`
    7. (非同期) イベント購読サービスが `avion.drop.drop.created` を消費:
        - avion-timeline: タイムライン更新
        - avion-user: UserStats.DropCount の増加
...
- **フロー 3: Drop削除 (Command)**
    ...
    8. DeleteDropCommandUseCase: EventPublisherを通じて `avion.drop.drop.deleted` イベントを発行
    9. DeleteDropCommandHandler → Gateway: `DeleteDropResponse {}`
    10. (非同期) イベント購読サービスが `avion.drop.drop.deleted` を消費:
        - avion-timeline: タイムラインから削除
        - avion-user: UserStats.DropCount の減少
...
- **フロー 4: リアクション追加 (Command)**
    ...
    7. AddReactionCommandUseCase: EventPublisherを通じて `avion.drop.reaction.created` イベントを発行
...
- **フロー 5: リアクション削除 (Command)**
    ...
    6. RemoveReactionCommandUseCase: EventPublisherを通じて `avion.drop.reaction.deleted` イベントを発行（キャッシュ更新はイベントハンドラが処理）
...
- **フロー 7: スケジュール投稿作成 (Command)**
    ...
    6. ScheduleDropCommandUseCase: EventPublisherを通じて `avion.drop.scheduled_drop.created` イベントを発行
...
- **フロー 10: スケジュール投稿自動実行 (Background Job)**
    ...
    3. ScheduledDropPublishJob: 各スケジュール投稿に対して:
        ...
        - EventPublisherを通じて `avion.drop.drop.created` イベントを発行
        - EventPublisherを通じて `avion.drop.scheduled_drop.published` イベントを発行
...
- **gRPC Services (`avion.DropService`):**
    ...
    - **Query Operations (参照系):**
        ...
        - ~~`GetTrendingHashtags(GetTrendingHashtagsRequest) returns (GetTrendingHashtagsResponse)`~~ // ※廃止予定: avion-search の同名エンドポイントに委譲
...
###### DropID
- **責務:** Dropの一意識別子を表現
- **属性:** ID値（UUID v7、Backend採番）
- **不変性:** 完全に不変
- **バリデーション:** 正しいUUID v7形式
...
###### Visibility
- **責務:** Dropの公開範囲を表現
- **値:** public, unlisted, followers_only, direct, private
- **不変性:** 完全に不変
- **バリデーション:** 定義された値のみ
...
- **PostgreSQL:**
    - `drops` table:
        - `id (UUID, PK)`
        - `user_id (UUID, FK to users.id, INDEX)`
        - `text (TEXT)`
        - `visibility (TEXT CHECK (visibility IN ('public', 'unlisted', 'followers_only', 'direct', 'private')))`
        - `media_ids (UUID[])`: `avion-media` で管理されるメディアIDの配列
        - `created_at (TIMESTAMP)`
        - `updated_at (TIMESTAMP)`
        - `is_edited (BOOLEAN DEFAULT FALSE)`
        - Index: `(user_id, created_at)`, `created_at`
    - `reactions` table:
        - `drop_id (UUID, PK, FK to drops.id)`
    ...
    - `scheduled_drops` table:
        ...
        - `published_drop_id (UUID)`: 公開後のDropID（公開後のみ）
...
- **Redis:**
    ...
    - **ホットDropキャッシュ (Sorted Set):**
        ...
        - 上位1000件のみ保持
    - **NATS JetStream Subjects:** `avion.drop.drop.created`, `avion.drop.drop.updated`, `avion.drop.drop.deleted`, `avion.drop.reaction.created`, `avion.drop.reaction.deleted`（Redis Pub/Subに代わりNATS JetStreamを使用）
    - **Stream:** `reaction_aggregation_queue` // 集計更新タスクキュー

#### 10.2.3 イベントペイロード設計

- **avion.drop.drop.created / avion.drop.drop.updated / avion.drop.drop.deleted:**
  ```json
  {
    "drop_id": "123456789",
    "user_id": "987654321",
...
- **avion.drop.reaction.created / avion.drop.reaction.deleted:**
  ```json
  {
    "drop_id": "123456789",
    "user_id": "987654321",
...
- **avion.drop.renote.created / avion.drop.renote.deleted:**
  ```json
  {
    "renote_id": "111111111",
    "drop_id": "123456789",
...
- **avion.drop.bookmark.created / avion.drop.bookmark.deleted:**
  ```json
  {
...
- **avion.drop.pin.created / avion.drop.pin.deleted:**
  ```json
  {
    "drop_id": "123456789",
    "user_id": "987654321",
...
- **avion.drop.favorite.created / avion.drop.favorite.deleted:**
  ```json
  {
...
- **avion.drop.draft.saved / avion.drop.draft.updated / avion.drop.draft.deleted:**
  ```json
  {
    "draft_id": "555555555",
    "user_id": "987654321",
...
- **avion.drop.scheduled_drop.created / avion.drop.scheduled_drop.updated / avion.drop.scheduled_drop.cancelled:**
  ```json
  {
...
- **avion.drop.scheduled_drop.published:**
  ```json
  {
    "scheduled_drop_id": "666666666",
    "drop_id": "777777777",
...
- **avion.drop.report.created / avion.drop.report.resolved:**
  ```json
  {
    "report_id": "777777777",
    "drop_id": "123456789",
    ...
    "description": "スパム投稿です",
    "status": "pending",
    "resolved_by": "admin123",  // avion.drop.report.resolvedの場合のみ
    "resolved_at": "2025-03-30T13:00:00Z",  // avion.drop.report.resolvedの場合のみ
    "created_at": "2025-03-30T12:00:00Z"
  }
  ```

- **avion.drop.impression.recorded:**
  ```json
  {
    "drop_id": "123456789",
...
#### 10.2.4 avion-dropとavion-activitypubの責務分離
...
- **監視/アラート:**
    - **メトリクス:**
        ...
        - NATS JetStream発行エラー/遅延。
        - 集計更新タスクのキュー長、処理時間。
        - ホットDropのキャッシュメモリ使用量。
    - **ログ:** CRUD操作ログ、リアクション操作ログ、エラーログ。
    - **トレース:** API呼び出し、DBアクセス、キャッシュアクセス、イベント発行のトレース。
    - **アラート:** gRPCエラーレート急増、高レイテンシ、DB/Redis接続障害、NATS JetStream発行失敗、高負荷時のDB集計クエリ遅延。

## 12. エラーハンドリング戦略

> **注意:** エラーコードは[共通エラーコード標準](../common/errors/error-codes.md)に準拠します。このサービスではプレフィックス `DROP` を使用します（命名規則: `DROP_[LAYER]_[ERROR_TYPE]`）。
...
```sql
CREATE TABLE drops (
    id UUID PRIMARY KEY, -- UUID v7はアプリケーション側で採番
    author_id UUID NOT NULL REFERENCES users(id),
    text TEXT NOT NULL CHECK (char_length(text) <= 500),
    visibility TEXT NOT NULL CHECK (visibility IN ('public', 'unlisted', 'followers_only', 'direct', 'private')),
...
CREATE INDEX idx_drops_author_id ON drops (author_id);
CREATE INDEX idx_drops_created_at ON drops (created_at DESC);
CREATE INDEX idx_drops_visibility ON drops (visibility) WHERE is_deleted = FALSE;
CREATE INDEX idx_drops_reply_to ON drops (reply_to_drop_id) WHERE reply_to_drop_id IS NOT NULL;
```
...
CREATE INDEX idx_reactions_drop_id ON reactions (drop_id);
...
CREATE INDEX idx_reaction_summaries_drop_id ON reaction_summaries (drop_id);
CREATE INDEX idx_reaction_summaries_count ON reaction_summaries (count DESC) WHERE count > 0;
```
...
CREATE INDEX idx_drop_media_attachments_drop_id ON drop_media_attachments (drop_id);
```
...
CREATE INDEX idx_drop_edit_history_drop_id ON drop_edit_history (drop_id);
```
...
CREATE INDEX idx_renotes_drop_id ON renotes (drop_id);
CREATE INDEX idx_renotes_user_id ON renotes (user_id);
CREATE INDEX idx_renotes_created_at ON renotes (created_at DESC);
```
...
CREATE INDEX idx_bookmarks_drop_id ON bookmarks (drop_id);
```
...
CREATE INDEX idx_pinned_drops_user_id ON pinned_drops (user_id);
```
...
CREATE INDEX idx_drafts_user_id_updated_at ON drafts (user_id, updated_at DESC);
```
...
CREATE INDEX idx_draft_media_attachments_draft_id ON draft_media_attachments (draft_id);
```
...
CREATE INDEX idx_scheduled_drops_user_id ON scheduled_drops (user_id);
CREATE INDEX idx_scheduled_drops_scheduled_at ON scheduled_drops (scheduled_at) WHERE status = 'scheduled';
```
...
CREATE INDEX idx_scheduled_drop_media_scheduled_drop_id ON scheduled_drop_media_attachments (scheduled_drop_id);
```
...
CREATE INDEX idx_drop_reports_status ON drop_reports (status);
CREATE INDEX idx_drop_reports_created_at ON drop_reports (created_at DESC) WHERE status = 'pending';
```
...
CREATE INDEX idx_drop_impressions_drop_id ON drop_impressions (drop_id);
CREATE INDEX idx_drop_impressions_created_at ON drop_impressions (created_at DESC);
```
...
CREATE INDEX idx_drop_analytics_drop_id ON drop_analytics (drop_id);
CREATE INDEX idx_drop_analytics_date ON drop_analytics (date);
```
...
CREATE INDEX idx_drop_hashtags_drop_id ON drop_hashtags (drop_id);
CREATE INDEX idx_drop_hashtags_hashtag_id ON drop_hashtags (hashtag_id);
```
...
CREATE INDEX idx_drop_mentions_drop_id ON drop_mentions (drop_id);
CREATE INDEX idx_drop_mentions_mentioned_user_id ON drop_mentions (mentioned_user_id);
```
...
### 15.1 イベント連携
avion-dropはNATS JetStreamを通じて以下のイベントを発行します。avion-activitypubはこれらのイベントを購読し、適切なActivityPubアクティビティに変換します。

- `avion.drop.drop.created`: Createアクティビティに変換
- `avion.drop.drop.updated`: Updateアクティビティに変換
- `avion.drop.drop.deleted`: Deleteアクティビティに変換
- `avion.drop.reaction.created`: Like/EmojiReactアクティビティに変換（対向先に応じて）
- `avion.drop.reaction.deleted`: Undo(Like/EmojiReact)アクティビティに変換
- `avion.drop.renote.created`: Announceアクティビティに変換
- `avion.drop.renote.deleted`: Undo(Announce)アクティビティに変換
- `avion.drop.poll.created`: Questionアクティビティに変換
- `avion.drop.poll.voted`: CreateアクティビティでNoteを送信（投票結果を含む）
...
### 16.3 編集制限
- **時間制限:** 作成後30分間のみ編集可能（設定可能）
- **回数制限:** v1では無制限（将来的に検討）

### 16.4 ActivityPub連携
- `avion.drop.drop.updated`イベント発行時、avion-activitypubが`Update`アクティビティを生成
...
- Drop Aggregate、基本CRUD API、PostgreSQLスキーマ作成
- DropRepository実装、基本的なDrop作成・取得・論理削除機能
- Visibility（公開範囲）によるアクセス制御
- NATS JetStreamイベント発行基盤
- リリース基準: ユニットテストカバレッジ85%以上、基本APIのレイテンシp99 < 200ms

### Phase 2: Reactions & Engagement (Week 3-4)
- Reaction Aggregate実装、リアクション追加・削除・集計API
- ReactionSummaryテーブルと非正規化集計
- Redisキャッシュ（reaction_counts、user_reactions）
- リリース基準: リアクション操作のレイテンシp99 < 150ms、集計取得p99 < 100ms

### Phase 3: Rich Content (Week 5-6)
- Poll Aggregate実装、投票作成・投票・結果取得API
- MediaAttachment関連機能、Renote（引用・リポスト）機能
- Bookmark/Favorite機能
- Drop編集機能（30分制限、編集履歴）
- コンテンツ警告（CW）機能
- リリース基準: 投票操作の正確性テスト合格、メディア添付のE2Eテスト合格

### Phase 4: Scheduled Posts & Drafts (Week 7-8)
- ScheduledDrop Aggregate実装、予約投稿・下書き管理API
- スケジュール投稿自動実行ジョブ（Kubernetes CronJob）
- Draft機能（保存・更新・公開変換）
- リリース基準: スケジュール実行遅延 < 1分、リトライ機構のテスト合格

### Phase 5: Moderation & Analytics (Week 9-10)
- DropReport（通報）機能
- センシティブマーク機能
- Drop閲覧記録・分析機能
- ハッシュタグ・メンション抽出（共有パッケージ連携）
- リリース基準: 通報フローのE2Eテスト合格、分析データ集計の正確性検証

### Phase 6: Integration & Optimization (Week 11-12)
- 他サービス（avion-timeline, avion-search, avion-activitypub, avion-notification）との連携テスト
- パフォーマンス最適化（キャッシュウォーミング、クエリ最適化）
- 負荷テスト（目標: 5,000 req/s）
- カオスエンジニアリングによる障害耐性検証
- リリース基準: サービス間連携のE2Eテスト合格、負荷テスト目標達成

## 19. Concerns / Open Questions (懸念事項・相談したいこと)

- **技術的負債リスク:**
    - ~~**物理削除の非可逆性:**~~ **解決済:** 論理削除方式を採用。DeletedAtに削除時刻を設定し、一定期間後にバッチジョブで物理削除する。これにより「ゴミ箱」機能への将来的な拡張も可能。
    - **イベントペイロード:** `avion.drop.drop.created` で全データを含めるのは、データサイズが大きい場合にネットワーク帯域や受信側サービスの負荷になる可能性がある。必要な情報のみに絞るか検討が必要。
...
## 20. Service-Specific Test Strategy (avion-drop固有テスト戦略)
...
func TestDropAggregate_CreateDrop_VisibilitySettings(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            ctx := ctxtimetest.SetFixedNow(context.Background(), time.Now())
            ...
            if tt.wantErr == nil {
                if drop == nil {
                    t.Fatal("drop should not be nil")
                }
                if diff := cmp.Diff(tt.visibility, drop.Visibility()); diff != "" {
                    t.Errorf("visibility mismatch (-want +got):\n%s", diff)
                }
                if diff := cmp.Diff(tt.userID, drop.UserID()); diff != "" {
...
- **UseCase Layer**: 85%以上（エラーケースを含む包括的テスト）
- **Infrastructure Layer**: 85%以上（外部依存を含む統合テスト）
- **Handler Layer**: 85%以上（APIエンドポイントの全パターン）
