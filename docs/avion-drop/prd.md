...
*   ハッシュタグの抽出・管理（`avion-search`との連携用）
    - Go共有パッケージ `pkg/textanalysis` を使用してDrop作成時にハッシュタグを抽出・正規化
    - 抽出済みハッシュタグを `DropCreatedEvent` の `ExtractedHashtags` フィールドに含めて発行
    - `avion-search` が同イベントを消費し、インデックス構築に利用
    - 不変条件: ハッシュタグは最大10個まで
*   メンションの抽出・管理（通知用）
    - Go共有パッケージ `pkg/textanalysis` を使用してDrop作成時にメンションを抽出・正規化
    - 抽出済みメンションを `DropCreatedEvent` の `ExtractedMentions` フィールドに含めて発行
    - `avion-notification` が同イベントを消費し、メンション通知を生成
    - 不変条件: メンションは最大20ユーザーまで
*   スケジュール投稿機能
    - 投稿予約（最大30日先まで）
    - 下書き保存・管理
    - 予約投稿の編集・キャンセル
...
*   **トレンドハッシュタグ集計:** ~~GetTrendingHashtags~~ は廃止。`avion-search` の `GetTrendingHashtagsQueryUseCase` に委譲済み。Gateway経由で `avion-search` に直接ルーティングする。
...
- **不変条件**:
  ...
  - リプライ元Dropが削除された場合もリプライ自体は残る（ビジネスルール：会話の継続性。孤立返信として表示）
  - Renote元Dropが削除された場合、Renoteは非表示化される（ビジネスルール：引用元の権利尊重）
  - 返信の削除時、その返信への子返信はカスケード削除せず孤立返信として残る（ビジネスルール：発言の保全）
  - 編集は投稿から30分以内、最大5回まで（ビジネスルール：編集の濫用防止）
  - ハッシュタグは最大10個まで（ビジネスルール：スパム防止）
  - メンションは最大20ユーザーまで（ビジネスルール：迷惑行為防止）
- **ドメインロジック**:
...
#### Thread Aggregate
**責務**: スレッド形式のコンテンツ管理とDrop順序制御を行う独立した集約
- **集約ルート**: Thread
- **不変条件**:
  - ThreadIDは変更不可（UUID v7、Backend採番）
  - スレッド内のDropは順序を持つ（ThreadPosition で管理）
  - スレッドの作成者のみがDrop追加・順序変更・分割・結合・閉鎖が可能
  - 閉鎖されたスレッドへのDrop追加は不可
  - スレッド内のDrop最大数は制限される（MaxThreadItems）
- **ドメインロジック**:
  - `addDrop(dropID, position)`: スレッドへのDrop追加（順序検証含む）
  - `reorderDrops(newOrder)`: スレッド内Drop順序の変更
  - `split(atPosition)`: スレッド分割（指定位置で2つのスレッドに分離）
  - `merge(otherThread)`: スレッド結合（権限確認、順序統合）
  - `close(closerID)`: スレッド閉鎖（以降のDrop追加を禁止）
  - `canModify(userID)`: 変更権限の判定（作成者確認）

#### Reply Aggregate
**責務**: 返信の管理と会話スレッドの構築を行う独立した集約
- **集約ルート**: Reply
- **不変条件**:
  - ReplyIDは変更不可（UUID v7、Backend採番）
  - 返信先DropIDは変更不可（参照整合性の保証）
  - 返信の深さ（ReplyDepth）は上限がある（MaxReplyDepth）
  - 返信元Dropが削除された場合も返信自体は残る（孤立返信として表示）
  - 返信の削除時、その返信への子返信は孤立返信として残る（カスケード削除しない）
- **ドメインロジック**:
  - `createReply(parentDropID, content)`: 返信作成（深さ制限検証含む）
  - `getReplyChain(depth)`: 返信チェーンの取得（指定深さまで）
  - `getConversationContext()`: 会話コンテキストの取得（ルートDropまで遡行）
  - `canReplyTo(parentDropID, userID)`: 返信可能判定（ブロック、プライバシー確認）
  - `markOrphaned()`: 孤立返信としてマーク（親Drop削除時）

#### ContentRelation Aggregate
**責務**: コンテンツ間の関係（メンション、ハッシュタグ、引用）を管理する独立した集約
- **集約ルート**: ContentRelation
- **不変条件**:
  - RelationIDは変更不可（UUID v7、Backend採番）
  - 関係タイプ（RelationType）は定義された値（mention, hashtag, quote, link）のいずれか
  - SourceDropIDとTargetDropID（またはTargetEntity）の組み合わせと関係タイプで一意
  - ソースDropが削除された場合、関連する関係も無効化される
- **ドメインロジック**:
  - `createRelation(sourceDropID, target, relationType)`: 関係の作成（重複検証含む）
  - `resolveTarget(target)`: 対象の解決（メンション先ユーザーの存在確認、ハッシュタグの正規化等）
  - `getRelationsByType(relationType)`: 関係タイプ別の取得
  - `invalidateRelations(sourceDropID)`: ソースDrop削除時の関係無効化
...
- **ContentRelationService**: コンテンツ間関係（メンション、ハッシュタグ、引用）管理のビジネスルール統括
...
**識別子関連**
- **DropID**: Dropの一意識別子（UUID v7、Backend採番）
- **ReactionID**: リアクションの一意識別子（UUID v7）
- **EventID**: イベントの一意識別子（冪等性保証用、UUID v7）
- **AuthorUserID**: Drop作成者のユーザーID
- **ReactorUserID**: リアクション実行者のユーザーID
- **UserID**: 汎用的なユーザーID
- **MediaID**: メディアサービスでの識別子
- **ScheduledDropID**: スケジュール投稿の一意識別子（UUID v7）

**Drop属性**
- **DropText**: Dropのテキスト内容を表現
  - 最大500文字（設定可能）、Unicode対応
...
- **ペイロード**:
  ...
  - ExtractedHashtags: []string（`pkg/textanalysis` で抽出済み）
  - ExtractedMentions: []UserID（`pkg/textanalysis` で抽出済み）
  - MediaAttachmentIDs: []MediaID
  - ReplyToDropID: *DropID（リプライ先、nilの場合はルート投稿）
...
- **ペイロード**:
  ...
  - DeletedBy: UserID
  - OccurredAt: time.Time
- **ビジネス意味**: 会話の一部が削除された（子返信は孤立返信として残る）

#### QuoteDropCreatedEvent
**発生条件**: 引用Dropが作成された時
- **ペイロード**:
  - QuoteDropID: DropID
  ...
  - AuthorID: UserID
  - OccurredAt: time.Time
- **ビジネス意味**: 既存コンテンツが引用され、新たな文脈が追加された

#### DropRepostedEvent
**発生条件**: Dropがリポスト（Renote）された時
- **ペイロード**:
  - RenoteID: RenoteID
  - OriginalDropID: DropID
  - RenoteUserID: UserID
  - OccurredAt: time.Time
- **ビジネス意味**: コンテンツが再共有され、拡散された
...
- **ペイロード**:
  ...
  - Hashtag: string
  - OccurredAt: time.Time
- **ビジネス意味**: コンテンツにタグが付与され、検索・分類可能になった

> **注記**: 上記はPRDレベルの主要イベント定義である。DesignDocでは46個のイベント（Core / Internal / Future の3カテゴリ）が定義されている。各イベントの詳細スキーマは [DesignDoc](./designdoc.md) および [共通イベントスキーマ定義](../common/events/event-schemas.md) を参照。
...
6.  DropValidationService でテキスト長（最大500文字）、文字エンコーディング、禁止語句をチェック
7.  PollValidationService で投票設定の検証（選択肢2-10個、期限5分-7日）
...
13. DropEventPublisher で `drop_created` イベントを NATS JetStream に発行
14. CreateDropResponse DTO を生成して返却（DropID、作成日時を含む）
15. (非同期) EventHandler が以下のサービスにイベントを伝播:
    - avion-timeline: タイムライン更新
    - avion-user: UserStats.DropCount の増加（`drop.created` イベント消費）
...
10. ReactionCleanupService で関連するすべてのReactionを論理削除
11. DropEventPublisher で `drop_deleted` イベントを NATS JetStream に発行
...
14. (非同期) EventHandler が以下のサービスにイベントを伝播:
    ...
    - avion-user: UserStats.DropCount の減少（`drop.deleted` イベント消費）
...
12. ReactionEventPublisher で `reaction_created` イベントを NATS JetStream に発行
...
10. ReactionEventPublisher で `reaction_deleted` イベントを NATS JetStream に発行
...
*   **本文:** 最大文字数制限を設けること (500文字、設定可能)。Unicode文字を正しく扱えること。
*   **公開範囲:** Dropごとに公開範囲 (public, unlisted, followers_only, direct, private) を設定できること。デフォルト値を設定できること。
*   **一意なID:** 各Dropにはシステム全体で一意なIDが付与されること (UUID v7、Backend採番)。
...
[共通テスト戦略](../common/testing-strategy.md)に準拠する。

*   **ユニットテスト**: 最小85%以上（推奨95%以上）
*   **クリティカルパス**: 95%以上（推奨100%）
*   **ドメインロジック**: 95%以上（推奨100%）
*   **統合テスト**: 80%以上（推奨90%以上）

## Release Plan

### Phase 1: Foundation (Week 1-2)
- Drop Aggregate実装、基本CRUD API
- PostgreSQLスキーマ作成、Visibility（公開範囲）制御
- NATS JetStreamイベント発行基盤

### Phase 2: Reactions & Engagement (Week 3-4)
- Reaction Aggregate実装、リアクション追加・削除・集計API
- Redisキャッシュ（reaction_counts TTL: 1時間）

### Phase 3: Rich Content (Week 5-6)
- Poll（投票選択肢2-10個）、Bookmark/Favorite機能
- Drop編集機能（30分制限、編集履歴）、CW機能
- MediaAttachment、Renote機能

### Phase 4: Scheduled Posts & Drafts (Week 7-8)
- ScheduledDrop（draft/scheduled/published/cancelled）
- 下書き保存・予約投稿自動実行ジョブ

### Phase 5: Moderation & Analytics (Week 9-10)
- DropReport（通報）、センシティブマーク
- 閲覧記録・分析、ハッシュタグ・メンション抽出

### Phase 6: Integration & Optimization (Week 11-12)
- サービス間連携テスト、パフォーマンス最適化
- 負荷テスト（5,000 req/s目標）

## 決まったこと（DesignDocで解決済み）

*   **Drop編集機能の仕様**: 投稿後30分以内のみ編集可能、最大5回まで、編集履歴を保存・表示する方式で確定
*   **Dropの削除方針**: 論理削除に確定（DeletedAtに削除時刻を設定、一定期間後にバッチで物理削除）
*   **リアクションデータのDBスキーマ設計**: reaction_summariesテーブル（専用集計テーブル）を導入し、非正規化によるO(1)取得を実現
*   **キャッシュ戦略**: 永続的Redisキャッシュとイベント駆動更新で統一。リアクション集計はRedis Hash、ユーザー別リアクションはRedis Set、ホットDropは優先的にメモリ保持。トランザクション内でDBとキュー更新を実施し整合性を保証
*   **大量リアクション集計パフォーマンス**: reaction_summariesテーブルの非正規化とインメモリキャッシュで解決。数万リアクションでもO(1)で取得可能
*   **引用/リポストのデータモデル**: Renote集約として独立管理。RenoteID、DropID（対象）、UserID、Comment（引用時のコメント、オプション）で構成。同一ユーザーによる同一Dropへの重複リノート不可、自己Renote不可を不変条件とする
*   **ハッシュタグ・メンション抽出方法**: Go共有パッケージ `pkg/textanalysis` を使用。Drop作成時に本サービスが抽出し、`DropCreatedEvent` に含めて発行。`avion-search` が同イベントを消費しインデックス構築。不変条件: ハッシュタグ最大10個、メンション最大20ユーザー
*   **スケジュール実行の実装方法**: Kubernetes CronJob（推奨）を採用。1分ごとにJobを起動し、公開時刻に達した投稿を検索・実行。同時実行数制限（セマフォ10）、リトライ機構（最大3回）、失敗時のステータス管理を含む
...
*   タイムゾーン対応（ユーザーのローカルタイムゾーンでの表示。v1ではUTC統一、v2で対応予定）
*   繰り返し投稿機能の実装有無（定期投稿）
*   下書きの自動保存機能の実装詳細
...
*   **UserStats更新（投稿数カウント）**: Drop作成時に `drop.created` イベント、削除時に `drop.deleted` イベントを NATS JetStream に発行する。avion-user がこれらのイベントを消費し、UserStats.DropCount を非同期で更新する。avion-drop の責務はイベント発行のみであり、UserStats の直接更新は行わない。イベント欠損時の整合性は avion-user 側の定期再計算バッチ（毎時実行）により保証される
*   **ハッシュタグ・メンション抽出**: テキスト解析（抽出・正規化）ロジックはGo共有パッケージ（`pkg/textanalysis` 等）として切り出す。本サービスはDrop作成時に共有パッケージを利用して抽出し、不変条件（ハッシュタグ最大10個、メンション最大20ユーザー）の検証を行う。`avion-search` も同じ共有パッケージを利用してインデックス構築する。これにより正規化ロジックの一貫性を保証する
*   **トレンドハッシュタグ集計**: `avion-search` の `GetTrendingHashtagsQueryUseCase` を正とする。本サービスの `GetTrendingHashtagsQueryHandler` は廃止し、Gateway経由で `avion-search` に委譲する
*   **センシティブ管理（3層構造）**: 本サービスはユーザー自己申告によるセンシティブマーク（`drop_marked_sensitive` イベント）を担当する。`avion-media` がメディアレベルのNSFW自動検出・フラグ管理、`avion-moderation` がプラットフォームポリシーに基づく強制適用（最終決定権）を担当する。優先度順: moderation強制適用 > media ML検出 > ユーザー自己申告
*   **コンテンツフィルタリング**: 非同期イベント検査方式を採用。Drop作成後に `DropCreatedEvent` を発行し、`avion-moderation` が非同期で検査する。違反検出時は `avion-moderation` が事後削除・制限アクションを実行する。本サービスの `DROP_USECASE_CONTENT_FILTERING_FAILED` エラーは廃止する
*   **ActivityPub変換**: `toActivityPubNote()` は本サービスから削除し、`avion-activitypub` の `ActivityPubTranslator`（アンチコラプションレイヤー）に移譲する。本サービスはドメインイベント（`DropCreatedEvent`）の発行のみを担う
*   **コミュニティ内投稿**: Topic内の投稿は本サービスのDropと同一モデルを使用する。`avion-community` はDropとTopic/Communityの関連付け（`community_id`, `topic_id`）のみを管理する
  - **補足**: コミュニティ内投稿においてDrop Aggregateは `community_id`/`topic_id` を直接保持しない。avion-communityの `CommunityPost` Entityが `Drop ID` と `Community/Topic ID` の関連付けテーブルを管理する。Drop側からは通常のDropと同一のライフサイクルで処理され、コミュニティとの関連はavion-community側が責任を持つ
