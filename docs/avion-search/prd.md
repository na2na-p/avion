...
検索処理を独立したステートレスマイクロサービスとして実装することで、MeiliSearchを主軸とした専門的な検索エンジン（障害時はPostgreSQL FTSへのフェイルオーバー対応）を効率的に活用し、他のサービスへの影響を最小限に抑えながら高速で関連性の高い検索結果を提供できる。
...
* **MeiliSearch連携（1stリリース）**:
  ...
  - 日本語検索最適化設定: **MeiliSearch v1.9 内蔵 Lindera**（IPA辞書ベース、Viterbiアルゴリズムによる形態素解析）を使用。Docker設定で `--features japanese` カスタムビルドオプションを指定
...
#### Community Search
* **コミュニティ検索機能**:
  - `avion-community` が発行する `community_created`、`community_updated`、`community_deleted` イベントを購読し、コミュニティ検索インデックスを管理
  - CommunitySearchDocument エンティティによるコミュニティ情報（名前、説明、カテゴリ、メンバー数等）のインデックス化
  - コミュニティ名、説明文、カテゴリ、トピックでの全文検索
  - メンバー数、アクティビティ頻度、作成日時によるランキング
  - プライバシー設定に基づく検索結果フィルタリング（公開コミュニティのみ検索可能、招待制コミュニティは参加者のみ検索可能）

#### Enhanced Search Features
* **ハッシュタグ検索とトレンド分析**:
  - ハッシュタグ抽出: `avion-drop` が `DropCreatedEvent` に `ExtractedHashtags` フィールドを含めて送信し、本サービスはこのフィールドを利用してハッシュタグインデックスを効率的に更新する（Drop本文の再パースが不要）。抽出ロジック自体はGo共有パッケージ（`pkg/textanalysis`）のHashtagExtractorを使用する
...
* **リアルタイムインデックス（厳密な意味）**: 若干のインデックス更新遅延を許容。**インデックス反映SLO: 投稿から検索可能になるまで5秒以内**を目標とする
* **ユーザー管理**: ユーザー情報の管理はavion-userの責任
* **投稿管理**: Drop情報の管理はavion-dropの責任
* **メディア処理**: メディアファイルの処理はavion-mediaの責任
* **E2E暗号化メッセージの本文検索**: E2E暗号化メッセージはサーバー側で復号不可のため、本サービスのスコープ外。クライアントサイドインデックスにより実行される。非暗号化メタデータ（送信者名、タイムスタンプ、会話名、添付ファイルの種類等）の検索インデックス管理のみ本サービスが担当する
...
* **他のAvionマイクロサービス**:
  ...
  - avion-message: 非暗号化メタデータの検索インデックス更新イベントの発行元（E2E暗号化メッセージ本文は対象外）
...
- **属性**:
  - DropID (雪花ID - 一意識別子)
  - SearchableText (最大5,000文字 - 検索対象テキスト)
...
- **EventID**: UUID v7形式（冪等性制御用）
- **SearchQueryID**: UUID v7形式（キャッシュキー生成用）

**検索条件属性**
- **SearchKeyword**: キーワード文字列（最大256文字、サニタイズ済み）
...
6. SearchQueryValidatorがキーワード長（最大256文字）、禁止文字、インジェクション攻撃をチェック
...
2. avion-dropがDrop作成完了後、drop_createdイベントをNATS JetStreamに発行
3. EventConsumerHandlerが専用NATS JetStream Consumerでイベントを購読
...
8. HashtagExtractorがDrop本文からハッシュタグを抽出・正規化（※ `DropCreatedEvent`には`ExtractedHashtags`フィールドが含まれ、avion-searchはこのフィールドを利用してハッシュタグインデックスを効率的に更新する（Drop本文の再パースが不要））
...
2. avion-userが設定更新後、UserSearchPrivacyUpdatedイベントをNATS JetStreamに発行
3. SearchPrivacyHandlerが専用NATS JetStream Consumerでイベントを受信
...
| 指標 | 目標値 |
|------|--------|
| API レスポンスタイム (p50) | < 150ms |
| API レスポンスタイム (p99) | < 800ms |
| 可用性 (SLA) | 99.9% |

### レイテンシ

* **検索API応答時間**: p50 100ms以下、p99 500ms以下（MeiliSearch性能に依存）
* **インデックス反映SLO**: 投稿から検索可能になるまで**5秒以内**（p50）。p99で30秒以下
...
* **障害対応**: MeiliSearch障害時のPostgreSQL FTSへの自動フェイルオーバー。フェイルオーバー判定条件は以下の通り:
  - **レイテンシ閾値**: MeiliSearchへの検索リクエストのp99レイテンシが2秒を超過した場合
  - **エラー率閾値**: 直近5分間のMeiliSearchリクエストのエラー率が10%を超過した場合
  - **接続不能**: MeiliSearchへの接続が3回連続で失敗した場合（ヘルスチェック間隔: 10秒）
  - **回復判定**: MeiliSearchのヘルスチェックが5回連続で成功し、かつp99レイテンシが500ms以下に回復した場合にフェイルバック
  - **サーキットブレーカー**: CLOSED（正常）→OPEN（フェイルオーバー）→HALF_OPEN（回復試行）の3状態で管理
...
* **テスト要件**: 単体テスト85%以上、統合テスト、性能テスト（負荷・ストレステスト）
...
* **MeiliSearch詳細設定**:
  - ~~日本語トークナイザーの具体的な選択~~ → **決定済み: MeiliSearch v1.9 内蔵 Lindera（IPA辞書ベース、Viterbiアルゴリズム）を採用。Docker設定で `--features japanese` カスタムビルドオプションを指定**
...
* **PostgreSQL FTS設定（2ndリリース以降）**:
  ...
  - **フォールバック精度（決定済み）**: MeiliSearch（Lindera）の精度を100%とした場合、PostgreSQL FTSは**70-80%程度の精度で許容する**。フォールバックは一時的な措置
* **検索バックエンド切り替え**:
...
* **検索の一元管理**: avion-search はAvionプラットフォームにおける検索インフラの中央管理サービスである。MeiliSearch および PostgreSQL FTS を用いた全文検索機能を一元的に提供する。各サービスは検索インデックスの更新イベントを NATS JetStream 経由で発行し、検索クエリの実行は avion-search の gRPC API を呼び出す形で統一する。これにより、検索ロジックの分散を防ぎ、検索品質・パフォーマンスの一貫した管理を実現する
* **テキスト解析**: ハッシュタグ・メンションの抽出・正規化ロジックはGo共有パッケージ（`pkg/textanalysis`）として切り出す。本サービスの `HashtagExtractor` / `MentionExtractor` と `avion-drop` の `extractHashtags()` / `extractMentions()` は共に共有パッケージの呼び出しに置き換える。これにより日本語ハッシュタグの正規化等のルール変更が1箇所で完結する
* **トレンドハッシュタグ集計**: 本サービスの `GetTrendingHashtagsQueryUseCase` を正とする。`avion-drop` の `GetTrendingHashtagsQueryHandler` は廃止し、Gateway経由で本サービスに委譲する
* **avion-message との検索責務境界**: E2E暗号化メッセージの本文検索はサーバー側で復号不可のため、avion-search のスコープ外とする。E2E暗号化メッセージの検索はクライアントサイドインデックスにより実行される。一方、非暗号化メタデータ（送信者名、タイムスタンプ、会話名、添付ファイルの種類等）の検索インデックス管理は avion-search に委譲する。avion-message はメッセージ本文を avion-search に送信しない
...
**期間**: 4-6週間
**スコープ**:
- Drop/User基本検索機能（MeiliSearch連携）
- イベント駆動インデックス更新（NATS JetStream）
- アクセス制御基盤
- 構造化ログ・OpenTelemetry基盤
- SearchBackend interfaceによる抽象化

**成果物**:
- `SearchDrops`, `SearchUsers` gRPC API
- 基本的なインデックス更新機能
- 日本語検索最適化設定
...
**期間**: 3-4週間
**スコープ**:
- 検索プライバシー制御（GDPR対応、オプトイン/オプトアウト）
