...
*   JWT検証（公開鍵によるローカル検証、結果のキャッシュ）
    *   **JWT検証責務の分離（決定事項）:** gatewayがJWT署名検証+有効期限チェックを実施し、判定結果を15分間Redisキャッシュする。avion-authは鍵発行・トークン生成のみを担当し、JWT検証は行わない
*   Bot認証サポート（APIキー検証）
...
*   **レート制限統合アルゴリズム（決定事項）:** 階層的評価を実施する。評価順序は global → IP → user → endpoint の順で、いずれかの層で制限を超過した時点でリクエストを拒否する。各層の制限は独立して管理され、最も厳しい制限が適用される
...
*   gRPC → HTTP（レスポンス変換）
*   WebSocket プロキシ（avion-messageへのリアルタイム通信中継）
...
#### メッセージングサービスへのルーティング
*   avion-messageへのgRPCルーティング（メッセージ送受信、会話管理、暗号化鍵交換等）
*   WebSocket接続のプロキシ（リアルタイムメッセージ配信、タイピングインジケーター、既読通知等）
*   メッセージング用WebSocket接続の認証（JWT検証後にWebSocketアップグレード）
*   WebSocket接続のレート制限（メッセージ送信頻度制限）
...
1.  クライアントがJWT付きでWebSocketアップグレードリクエストを送信
2.  `avion-gateway` はJWTを検証（ローカルキャッシュ → 公開鍵検証）
3.  レート制限チェック（ユーザー単位）
4.  WebSocket接続を確立し、`avion-message` へプロキシ
5.  リアルタイムメッセージ配信、タイピングインジケーター、既読通知を中継
6.  接続切断時に適切なクリーンアップを実施

### メッセージングAPIリクエスト

1.  クライアントがJWT付きでメッセージング関連APIリクエストを送信
2.  `avion-gateway` はJWTを検証
3.  レート制限チェック（ユーザー単位）
4.  `avion-message` へgRPCでリクエストを転送（会話作成、メッセージ送信、暗号化鍵交換等）
5.  レスポンスをHTTPに変換してクライアントに返却
...
*   **ID生成方式:** UUID v7
*   **レート制限アルゴリズム:** Sliding Windowを採用。階層的評価（global → IP → user → endpoint）で最も厳しい制限を適用
*   **サーキットブレーカー:** sony/gobreakerを採用
*   **テストカバレッジ目標:** ユニットテスト85%以上、クリティカルパス95%以上
