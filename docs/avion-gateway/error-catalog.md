...
| GATEWAY_USECASE_BOT_NOT_FOUND | 404 | NOT_FOUND | Bot認証: Botが見つかりません | Bot IDを確認してください |
| GATEWAY_USECASE_INVALID_CLIENT_CREDENTIALS | 401 | UNAUTHENTICATED | Bot認証: クライアント認証情報が無効です | クライアントIDとシークレットを確認してください |
| GATEWAY_USECASE_SCOPE_NOT_ALLOWED | 403 | PERMISSION_DENIED | Bot認証: 要求スコープが許可されていません | 許可されたスコープを確認してください |
...
| GATEWAY_HANDLER_ACTIVITYPUB_SIGNATURE_INVALID | 401 | UNAUTHENTICATED | ActivityPub署名検証に失敗しました | HTTP Signatureヘッダーと署名鍵を確認してください |
...
| GATEWAY_HANDLER_QUERY_DEPTH_EXCEEDED | 400 | INVALID_ARGUMENT | GraphQLクエリ深度制限を超過しました | クエリのネスト深度を浅くしてください |
...
| エラーコード | HTTPステータス | gRPCステータス | 説明 | 対処法 |
|------------|--------------|--------------|------|--------|
| GATEWAY_HANDLER_WEBSOCKET_PROXY_BACKEND_UNAVAILABLE | 503 | UNAVAILABLE | WebSocketプロキシ先（avion-message）が利用できません | メッセージングサービスの状態を確認してください |
| GATEWAY_HANDLER_WEBSOCKET_PROXY_CONNECTION_LIMIT | 429 | RESOURCE_EXHAUSTED | WebSocket接続数の上限に達しました | 不要な接続を閉じてから再試行してください |
| GATEWAY_HANDLER_WEBSOCKET_PROXY_AUTH_FAILED | 401 | UNAUTHENTICATED | WebSocketプロキシ接続の認証に失敗しました | 認証トークンを確認してください |
