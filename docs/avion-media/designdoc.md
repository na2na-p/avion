...
- **ID生成:** UUID v7
- **画像処理:** libvips (Go バインディング: `github.com/h2non/bimg`)
  - CGo依存: Dockerビルドに `libvips-dev` パッケージが必要
- **動画処理:** ffmpeg（動画サムネイル生成、将来の変換処理用）
- **サムネイル生成:** libvips (bimg) による高速サムネイル生成

詳細な実装パターンおよびライブラリの使用方法については、[ガイドライン](../common/architecture/go-backend-framework.md)を参照してください。

### Dockerビルド例

libvipsはCGo依存のため、Dockerビルド時に`libvips-dev`パッケージのインストールと`CGO_ENABLED=1`の設定が必要です。
...
RUN go mod download
COPY . .
RUN CGO_ENABLED=1 go build -o /avion-media ./cmd/server

FROM debian:bookworm-slim
RUN apt-get update && apt-get install -y libvips && rm -rf /var/lib/apt/lists/*
COPY --from=builder /avion-media /avion-media
ENTRYPOINT ["/avion-media"]
```
...
- **Media Processing (libvips/bimg ベース):**
  - ImageProcessor: 画像処理実装（libvips/bimg使用）
  - VideoProcessor: 動画処理実装（ffmpeg使用）
  - AudioProcessor: 音声処理実装（ffmpeg使用）
  - ThumbnailGenerator: サムネイル生成実装（libvips/bimg使用）
  - GifProcessor: GIF処理実装（libvips/bimg使用）
  - SvgToImageConverter: SVG→PNG変換実装（libvipsのSVG対応を使用）
...
- **集約ルート**: Media
- **不変条件**:
  - MediaIDは変更不可（UUID v7）
  - ファイルサイズ上限：画像20MB、動画40MB、音声40MB
...
- **主要コンポーネント:**
    ...
    - `avion-drop (Go)`: Drop削除イベント発行元 (NATS JetStream)。
...
func NewMediaID() MediaID {
    return MediaID{value: "media_" + generateUUIDv7()}
}
...
func NewTaskID(taskType string) TaskID {
    return TaskID{value: fmt.Sprintf("task_%s_%s", taskType, generateUUIDv7())}
}
...
type CDNConfig struct {
    ...
    PurgeAPIURL    string        `env:"CDN_PURGE_API_URL" required:"false"` // 緊急削除用CDN purge API URL
}
...
- **フロー 4: メディア削除 (イベント駆動) (Command)**
    1. DropDeletedEventHandler: NATS JetStreamサブジェクト `avion.drop.drop.deleted` からイベント受信 (Payload: { ..., media_ids: [...] })
...
#### イベント購読
    - NATS JetStream Subjects: `avion.drop.drop.deleted` (購読)
...
CREATE TABLE media_folders (
    ...
    CONSTRAINT folder_depth_check CHECK (depth >= 0 AND depth <= 5),
...
- ドメインイベント → NATS JetStream で発行（DBには保存しない）
...
- **メモリ不足**: 大容量画像処理時
- **処理ライブラリエラー**: libvips(bimg)/ffmpegの異常終了
...
### 15.1. avion-drop との連携

**Purpose:** Dropの作成・削除時のメディア使用状況管理とライフサイクル連携

**Integration Method:** NATS JetStream Events
...
**Events Published:**
- `avion.media.upload.completed`: メディアアップロード完了時
- `avion.media.processing.completed`: サムネイル生成等の処理完了時
- `avion.media.upload.deleted`: メディア削除完了時
- `avion.media.cache.created`: リモートメディアキャッシュ作成時
- `avion.media.usage.updated`: 使用状況変更時
...
### Phase 1: MVP (v0.1.0)
- 画像アップロード（Presigned URL方式）、サムネイル自動生成
- S3互換ストレージ保存、CDN配信
- ファイルバリデーション、NSFWフラグ、ALTテキスト管理
- PostgreSQL 17 によるメタデータ管理
- NATS JetStream によるイベント配信基盤

### Phase 2: Enhanced Media (v0.2.0)
- 動画サムネイル、音声MP3変換、GIF静止画変換
- バッチアップロード、メディア使用状況追跡
- ユーザードライブ機能（フォルダ管理、容量管理）

### Phase 3: Federation & Advanced (v0.3.0)
- リモートメディアキャッシュ（ActivityPub連携）
- SVGカスタム絵文字PNG変換、メディアコレクション
- ロールベースストレージ容量管理、管理者API

### Phase 4: Accessibility & Optimization (v0.4.0)
- AI ALTテキスト自動生成、音声説明ファイル添付
- 高コントラストモード対応、処理エラー詳細通知
- パフォーマンス最適化
...
- SVG→PNG変換のセキュリティリスク（libvipsのSVG対応を使用。XXE攻撃、無限ループ等の対策が必要）。
...
## 18. Service-Specific Test Strategy (サービス固有のテスト戦略)
...
func TestImageProcessor_ProcessImage(t *testing.T) {
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            if diff := cmp.Diff(tt.expectedFormat, result.Format); diff != "" {
                t.Errorf("Format mismatch (-want +got):\n%s", diff)
            }
            if diff := cmp.Diff(tt.expectedSize, result.Dimensions); diff != "" {
...
func TestFileTypeValidator_ValidateFileType(t *testing.T) {
    tests := []struct {
        name     string
        fileData []byte
        filename string
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            got, err := validator.ValidateFileType(tt.fileData, tt.filename)
            ...
            if diff := cmp.Diff(tt.want, got); diff != "" {
                t.Errorf("detectedType mismatch (-want +got):\n%s", diff)
            }
        })
    }
}
...
#### 18.2.2. Integration Tests (統合テスト)
...
#### 18.2.3. Async Task Processing Tests (非同期タスク処理テスト)
...
#### 18.2.4. Performance Tests (パフォーマンステスト)
...
#### 18.2.5. Security Tests (セキュリティテスト)
...
### 18.3. Test Data Management (テストデータ管理)
...
### 18.4. Test Environment Setup (テスト環境セットアップ)
...
### 18.5. Continuous Integration (CI/CD)
