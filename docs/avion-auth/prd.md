...
*   **JWT有効期限:** アクセストークン1時間、リフレッシュトークン7日間
...
*   **鍵ローテーション:** JWT署名鍵は30日ごと、認証関連暗号化鍵（TOTPシークレット暗号化鍵、リカバリーコード暗号化鍵等）は90日ごと。E2Eメッセージ暗号化鍵はavion-messageが独立管理するため本サービスの管理対象外
...
*   [zitadel/oidc (GitHub)](https://github.com/zitadel/oidc) - OIDC RP/OP実装ライブラリ（OpenID Foundation認証済み）
*   WebAuthn/FIDO2 仕様
*   TOTP (RFC 6238)
*   JWT (RFC 7519)
...
*   セッション失効管理
*   公開鍵提供エンドポイント（JWKS）
*   リフレッシュトークン（DesignDocでテーブル設計完了、実装確定）
*   デバイスセッション管理
...
*   ポリシーベース認可（CEL: Common Expression Languageによるポリシー評価）
*   認可結果のキャッシング（高速化）
*   Bot用スコープ管理と検証
*   リソース所有者判定
...
*   Authorization Code Flow + PKCE（サードパーティアプリ用、zitadel/oidc OPパッケージで実装）
*   OIDC Discovery Endpoint（/.well-known/openid-configuration）
*   トークンエンドポイント（/oauth/token）
*   IDトークン発行（OpenID Connect標準クレーム準拠）
*   スコープ検証とトークン発行
*   クライアント管理（登録、無効化）

> **注記**: 上記はAvionがOP（OpenID Provider）として機能する場合の実装範囲を指す。
> 外部IdP（Google、GitHub等）をRP（Relying Party）として利用する外部ログイン連携は、
> zitadel/oidcのRPパッケージで対応する方針が決定済み（「決まったこと」セクション参照）。
> Phase 2で実装予定。

#### サービス間認証
*   APIKeyManager Aggregate（サービス間認証用APIキーのライフサイクル管理）
*   ServiceAccount Entity（サービスアカウントの作成・管理）
*   APIキーの発行・検証・失効管理

#### サービス間連携
*   avion-system-admin 連携（監査ログの転送・集約）
...
*   **E2E暗号化鍵管理:** Signal ProtocolによるE2Eメッセージ暗号化鍵（Identity Key、Signed Pre Key、One-Time Pre Key等）はavion-messageが独立管理。avion-authの鍵管理責務はJWT署名鍵、セッション暗号化鍵、認証・認可関連の暗号化鍵に限定される
...
- **不変条件**:
  ...
  - 同時セッション上限10、LRU方式で最古のセッションを自動失効
- **ドメインロジック**:
...
**識別子関連**
- **UserID**: ユーザーの一意識別子（UUID v7）
- **SessionID**: セッションの一意識別子（UUID v7）
- **ClientID**: OAuthクライアントID（ランダム文字列）
- **KeyID**: 署名鍵の識別子（kid）
- **PolicyID**: ポリシーの一意識別子（UUID v7）
...
- **実装要件**: パスワードハッシュには指定されたパラメータ（memory=64MB、iterations=3、parallelism=4）でArgon2idを使用する。JWT署名は最小2048ビット鍵でRS256を使用する必要がある。保存時の機密データ（リカバリーコード、TOTPシークレット）はAES-256-GCMを使用して暗号化する必要がある。署名鍵（30日サイクル）と暗号化鍵（90日サイクル）の適切な鍵ローテーションを実装する。
...
### テストカバレッジ
*   全体カバレッジ: 85%以上
*   クリティカルパス（認証フロー、JWT検証、認可判定）: 95%以上
...
*   **OIDCライブラリ:** zitadel/oidc v3を採用（OpenID Foundation認証済み、RP+OP両対応）
*   **サードパーティアプリ対応:** zitadel/oidcのOPパッケージでAuthorization Code Flow + PKCEを実装。`op.Storage`インターフェースを実装し、既存の認証・認可ドメインロジックと統合する
*   **サービス構成:** 認証（AuthN）と認可（AuthZ）は1サービスに統一を維持。内部を`oidc/`・`authn/`・`authz/`の3モジュール構成で責務分離する
*   **外部IdP連携方針:** OIDC対応のIdP（Google、GitHub等）はzitadel/oidcのRPパッケージで対応する方針
*   **リフレッシュトークン:** DesignDocでテーブル設計（`refresh_tokens`テーブル）およびUseCase（`RefreshTokenCommandUseCase`）の設計完了。実装確定
*   **ポリシー記述言語:** CEL（Common Expression Language）を採用。ポリシー条件式の評価エンジンとして使用する
*   **外部IdP連携実装時期:** Phase 2で実装。zitadel/oidcのRPパッケージを使用し、Google、GitHub等のOIDC対応IdPとの連携を実装する
