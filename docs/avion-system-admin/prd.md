...
| 操作 | p50 | p99 | 備考 |
|------|-----|-----|------|
| 危険操作実行 | < 2s | < 10s | 承認フロー含むため高レイテンシ許容 |
| 統計ダッシュボード表示 | < 200ms | < 500ms | |
| 設定取得 | < 50ms | < 100ms | キャッシュ活用 |
| 監査ログ書き込み | < 10ms | < 50ms | 非同期処理 |
| 権限チェック | < 5ms | < 20ms | インメモリキャッシュ |
| レート制限判定 | < 1ms | < 5ms | Redis活用 |
...
* **avion-auth からの監査ログ受信**: `avion-auth` は認証・認可に関する監査ログ転送イベント（ログイン試行、MFA操作、トークン発行・失効、権限変更等）をNATS JetStream経由で発行し、`avion-system-admin` がそれを購読・受信して集約する。これにより、認証・認可領域の監査証跡をシステム横断的な監査ログと統合し、統合検索APIによる一元的な検索・分析を可能にする。`avion-auth` 側でも90日間の監査ログ保持を行うが、長期保存とコンプライアンスレポートへの統合は `avion-system-admin` が担当する

#### 統合検索API仕様（概要）

avion-system-adminが提供する統合検索APIの概要:

- **エンドポイント**: gRPC `SearchAuditLogs` (avion-system-admin が提供)
- **検索対象**:
  - 本サービスの管理者操作監査ログ（直接クエリ）
  - avion-authの認証・認可監査ログ（NATS JetStream経由で受信・集約済みデータを直接クエリ）
  - avion-moderationのモデレーション監査ログ（gRPC経由で問い合わせ）
- **検索パラメータ**:
  - 日時範囲、操作者ID、操作種別、対象リソースID、重要度レベル
- **レスポンス**: 両系統のログを時系列で統合した結果を返却
- **アクセス制御**: `super_admin` または `audit_viewer` 権限が必要
- **レート制限**: 10 req/sec per user
- **注意事項**: avion-moderationのハッシュチェーン保護は維持され、検索時にチェーンの完全性検証結果も返却される

詳細な API仕様は DesignDoc のユースケース定義を参照。
