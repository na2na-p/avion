...
- **一言で:** AvionのWebフロントエンドをReact + Vite + React Router v6によるPure Client-Side SPAとして構築し、CDN経由で配信します。GraphQLとSSEはavion-gateway経由で利用します。
- **目的:** ユーザーに直感的なインターフェースを提供し、高速なSPA体験とCDN配信による優れたパフォーマンスを実現します。
...
- React + Vite + React Router v6によるPure Client-Side SPAとして構築し、CDN配信による高速な体験を実現。
...
- **実装要件**: クライアントサイドアプリケーションとして、avion-webは包括的なXSS防止を実装する必要があります。これには、適切なReactコンポーネントのサニタイゼーション、厳格なContent Security Policyヘッダー、ユーザー生成コンテンツの安全な処理、およびすべての外部データソースの検証が含まれます。マークダウンコンテンツのサニタイゼーション、DOMベースのXSSの防止、動的HTMLレンダリングのセキュリティ確保に特別な注意を払う必要があります。

### CSRF保護
- **ガイドライン**: [../common/security/csrf-protection.md](../common/security/csrf-protection.md)
- **実装要件**: すべての状態変更操作に対してダブルサブミットクッキーパターンを実装し、APIルートでOriginヘッダーを検証し、SameSiteクッキー属性を使用し、GraphQLミューテーションにCSRFトークンを含めることを確実にします。avion-gatewayは、バックエンドサービスにリクエストを転送する前にトークンを検証する必要があります。
...
avion-webは、ユーザーの好みや利用シーンに応じて選択可能な2つのUIモードを提供します。これらのモードは動的に切り替え可能で、設定はデバイス間で同期されます。

### 6.2. Deck Mode (デッキモード)

#### 概要
TweetDeck/Misskeyデッキモードに類似した多カラムレイアウトで、パワーユーザー向けに最適化されています。複数の情報ストリームを同時に監視し、効率的に情報を処理できます。

#### 主要機能
- **パネル管理**
  - ユーザーが任意のパネル（カラム）を追加・削除・並び替え可能
  - ドラッグ&ドロップによる直感的な配置変更
  - パネル幅の調整（固定幅: 320px, 400px, 480px / 可変幅: フレキシブル）
  - パネルごとの最小化・最大化機能
  - パネル設定のプリセット保存・読み込み機能
...
- **スクロール・ナビゲーション**
  - 各パネルが独立してスクロール
  - 水平スクロールでパネル間を移動
  - キーボードショートカット対応（Ctrl+左右矢印でパネル移動）
  - タッチジェスチャー対応（スワイプでパネル切り替え）

- **リアルタイム更新**
  - パネルごとの自動更新間隔設定（5秒〜60秒）
  - 新着通知のバッジ表示
  - リアルタイムストリーミング対応

### 6.3. Classic Mode (クラシックモード)

#### 概要
従来のTwitterのようなシングルカラムレイアウトで、カジュアルユーザーやモバイルユーザー向けに最適化されています。

#### 主要機能
- **レスポンシブデザイン**
  - モバイル（〜640px）: シングルカラム、下部ナビゲーション
  - タブレット（640px〜1024px）: サイドバー + メインコンテンツ
  - デスクトップ（1024px〜）: 左サイドバー + メインコンテンツ + 右サイドバー

- **ナビゲーション構造**
  - 左サイドバー: メインナビゲーション（ホーム、検索、通知、メッセージ、プロフィール）
  - 右サイドバー: トレンド、おすすめユーザー、広告スペース
  - モバイル: 下部タブナビゲーション
...
### 6.4. Cross-Device Settings Sync (デバイス間設定同期)
...
#### 同期メカニズム
- **データ保存**
  - avion-userサービスのユーザー設定APIを利用
  - 設定はJSONとしてPostgreSQLに永続化
  - 設定サイズ制限: 最大1MB
...
### 6.5. Technical Implementation (技術的実装詳細)

#### データモデル (TypeScript型定義)

```typescript
// UIモード設定の型定義
interface UIModeSettings {
  mode: 'deck' | 'classic';
  deckConfig?: DeckConfiguration;
  classicConfig?: ClassicConfiguration;
  syncEnabled: boolean;
  lastUpdated: string;
  version: number;
}
...
type PanelWidth = 'narrow' | 'medium' | 'wide' | 'flexible';
...
interface PanelSettings {
  ...
  userId?: string; // for user_profile panels
  listId?: string; // for list panels
  searchQuery?: string; // for search panels
}

interface Filter {
  type: 'include' | 'exclude';
  field: 'content' | 'author' | 'hashtag' | 'media_type';
  value: string;
  caseSensitive?: boolean;
}

// クラシックモード設定
interface ClassicConfiguration {
...
interface UIStore {
  ...
  setMode: (mode: 'deck' | 'classic') => void;
  toggleMode: () => void;
  
  // Actions - Deck Mode
  addPanel: (panel: Panel) => void;
  removePanel: (panelId: string) => void;
  updatePanelPosition: (panelId: string, newPosition: number) => void;
  updatePanelSettings: (panelId: string, settings: Partial<PanelSettings>) => void;
  updatePanelWidth: (panelId: string, width: PanelWidth) => void;
  ...
  updateClassicConfig: (config: Partial<ClassicConfiguration>) => void;
  ...
  enableSync: () => void;
  disableSync: () => void;
  handleSyncConflict: (local: UIModeSettings, remote: UIModeSettings) => void;
}
    ...
    persist(
      immer((set, get) => ({
        ...
        toggleMode: () => set((state) => {
          state.mode = state.mode === 'deck' ? 'classic' : 'deck';
          get().syncSettings();
        }),
        
        // Deck Mode Actions
        addPanel: (panel) => set((state) => {
          state.deckConfig.panels.push({
            ...panel,
            position: state.deckConfig.panels.length,
          });
          get().syncSettings();
        }),
        
        removePanel: (panelId) => set((state) => {
          const index = state.deckConfig.panels.findIndex(p => p.id === panelId);
          if (index !== -1) {
            state.deckConfig.panels.splice(index, 1);
            // Reorder positions
            state.deckConfig.panels.forEach((panel, i) => {
              panel.position = i;
            });
            get().syncSettings();
          }
        ...
        updatePanelPosition: (panelId, newPosition) => set((state) => {
          const panel = state.deckConfig.panels.find(p => p.id === panelId);
          if (panel) {
        ...
        updatePanelSettings: (panelId, settings) => set((state) => {
          const panel = state.deckConfig.panels.find(p => p.id === panelId);
          if (panel) {
            panel.settings = { ...panel.settings, ...settings };
            get().syncSettings();
          }
        }),
        
        reorderPanels: (startIndex, endIndex) => set((state) => {
          const [removed] = state.deckConfig.panels.splice(startIndex, 1);
          state.deckConfig.panels.splice(endIndex, 0, removed);
          // Update positions
          state.deckConfig.panels.forEach((panel, i) => {
            panel.position = i;
          });
        ...
        syncSettings: async () => {
          ...
          try {
            const settings: UIModeSettings = {
              mode: get().mode,
              deckConfig: get().deckConfig,
        ...
        loadSettings: async () => {
          try {
            ...
            const settings: UIModeSettings = await response.json();
            
            set((state) => {
              state.mode = settings.mode;
              if (settings.deckConfig) {
                state.deckConfig = settings.deckConfig;
              }
              if (settings.classicConfig) {
                state.classicConfig = settings.classicConfig;
              }
              state.syncStatus = 'synced';
              state.lastSyncTime = new Date().toISOString();
            });
      ...
      })),
      {
        ...
        partialize: (state) => ({
          ...
          classicConfig: state.classicConfig,
        }),
      }
    )
  )
...
├── ui-modes/
│   ├── DeckMode/
│   │   ├── DeckContainer.tsx          # Main container with horizontal scroll
│   │   ├── Panel.tsx                  # Individual panel component
│   │   ├── PanelHeader.tsx           # Panel header with controls
│   │   ├── PanelContent.tsx          # Panel content wrapper
│   │   ├── PanelSettings.tsx         # Panel settings modal
│   │   ├── AddPanelButton.tsx        # Button to add new panels
│   │   ├── PanelPresets.tsx          # Preset management UI
│   │   └── panels/                   # Panel type specific components
│   │       ├── TimelinePanel.tsx
│   │       ├── NotificationPanel.tsx
│   │       ├── SearchPanel.tsx
│   │       └── ...
│   ├── ClassicMode/
│   │   ├── ClassicLayout.tsx         # Main layout container
│   │   ├── MainContent.tsx           # Central content area
│   │   ├── Sidebar.tsx               # Left navigation sidebar
│   │   ├── TrendsSidebar.tsx         # Right trends sidebar
│   │   └── MobileNav.tsx             # Bottom navigation for mobile
│   ├── ModeSwitch.tsx                # Mode toggle component
│   └── SettingsSync.tsx              # Sync status indicator
```
...
type Mutation {
  updateUISettings(input: UISettingsInput!): UISettings!
  saveDeckPreset(input: DeckPresetInput!): DeckPreset!
  deleteDeckPreset(id: ID!): Boolean!
  setDefaultDeckPreset(id: ID!): DeckPreset!
}
...
type Query {
  ...
  sharedDeckPresets(category: PresetCategory): [SharedDeckPreset!]!
}

# Real-time subscriptions
type Subscription {
  uiSettingsUpdated: UISettings!
  panelDataUpdated(panelId: ID!): PanelData!
}

# Input types
input UISettingsInput {
  mode: UIMode!
  deckConfig: DeckConfigurationInput
  classicConfig: ClassicConfigurationInput
}

input DeckConfigurationInput {
  panels: [PanelInput!]!
...
#### Deck Mode Optimization
- **Virtual Scrolling**: react-windowを使用して大量のパネルでもパフォーマンスを維持
- **Lazy Loading**: 表示範囲外のパネルは遅延ロード
- **Independent Data Fetching**: 各パネルが独立したデータフェッチとキャッシング
- **Memoization**: React.memoとuseMemoを活用したレンダリング最適化
- **Debounced Updates**: パネル設定変更時の過剰な再レンダリング防止

#### Classic Mode Optimization
- **Intersection Observer**: 無限スクロールの効率的な実装
- **Image Lazy Loading**: ビューポート外の画像を遅延ロード
- **Responsive Images**: デバイスに応じた適切な画像サイズ配信

#### Sync Optimization
- **Differential Sync**: 変更差分のみを同期
- **Compression**: 設定データのgzip圧縮
- **Service Worker Cache**: オフライン対応とキャッシュ戦略
- **Optimistic Updates**: UIの即座更新とバックグラウンド同期
...
### 7.4. Frontend CQRS Implementation (フロントエンドCQRS実装)
...
#### 7.4.2. Command Implementation（更新系実装）
...
#### 7.4.3. Query Implementation（参照系実装）
...
#### 7.4.4. State Management with CQRS（状態管理とCQRS）
...
#### 7.4.5. React Hooks for CQRS（CQRSのためのReactフック）
...
#### 7.4.6. Optimistic Updates（楽観的更新）
...
#### 7.4.7. Cache Invalidation Strategy（キャッシュ無効化戦略）
...
### 7.8. External Service インターフェース詳細
...
export class ErrorReporter {
  ...
  static getInstance(): ErrorReporter {
    if (!this.instance) {
      // 環境変数で選択可能
      const reporterType = import.meta.env.VITE_ERROR_REPORTER_TYPE || 'internal';
...
1. External App → Gateway → avion-auth: 認可リクエスト
2. avion-auth: 認可セッション作成、session_id発行
...
7. GrantOAuthConsentCommandUseCase または DenyOAuthConsentCommandUseCase:
   - Client → Gateway → avion-auth: 同意結果送信
8. avion-auth: 同意結果に基づいて処理
   - 同意の場合: 認可コード発行
   - 拒否の場合: エラーレスポンス
9. avion-web → External App: コールバックURLへリダイレクト

### フロー 7: デッキモードへの切り替えとパネル設定
1. User: 設定画面でUIモード切り替えを選択
2. ToggleModeCommandUseCase: モード変更処理開始
3. UIStore: モードを'deck'に変更
4. Client → Gateway → User Service: 設定保存API呼び出し
5. DeckContainer: デッキレイアウトレンダリング
6. LoadPanelsQueryUseCase: 保存済みパネル設定取得
7. Panel Components: 各パネルの初期化
8. PanelDataFetchService: パネルごとのデータ取得開始
9. ViewStateAggregate: 各パネルのUI更新

### フロー 8: デッキモードでのパネル追加と設定
1. User: 「パネル追加」ボタンクリック
2. AddPanelModal: パネルタイプ選択UI表示
3. User: パネルタイプと初期設定を選択
4. AddPanelCommandUseCase: パネル作成処理
5. UIStore: 新規パネルを配列に追加
6. DragDropService: ドラッグ可能要素として登録
7. Client → Gateway: パネル設定同期
8. PanelComponent: 新規パネルレンダリング
9. PanelDataFetchService: パネル専用データストリーム開始

### フロー 9: クラシックモードでのレスポンシブ対応
1. WindowResizeObserver: ビューポートサイズ変更検知
2. ResponsiveLayoutService: ブレークポイント判定
3. ClassicLayout: レイアウト構造変更
   - モバイル: 下部ナビゲーション表示
   - タブレット: サイドバー折りたたみ
   - デスクトップ: フルレイアウト表示
4. ViewStateAggregate: UIコンポーネント再配置
5. MediaQueryService: CSS変数更新

### フロー 10: デバイス間での設定同期
1. Device A: UIモード/パネル設定変更
2. SyncSettingsCommandUseCase: 同期処理開始
3. Client A → Gateway → User Service: 設定更新API
4. User Service → Redis Pub/Sub: 設定変更イベント発行
5. Gateway → SSE → Device B: リアルタイムイベント配信
6. Device B: SettingsSyncService: イベント受信
7. ConflictResolutionService: タイムスタンプ比較
8. UIStore (Device B): 設定更新
9. ViewStateAggregate (Device B): UI自動反映

### ユーザーストーリー: パワーユーザーとしてのデッキモード利用
**シナリオ:** マーケティング担当者が複数のブランドアカウントを管理
1. User: デッキモードを有効化
2. User: ブランドAのタイムラインパネル追加
3. User: ブランドBのメンションパネル追加
4. User: 業界キーワードの検索パネル追加
5. User: 競合分析用のユーザープロフィールパネル追加
6. System: 各パネルが独立して自動更新
7. User: 重要な情報を見逃さずに効率的に監視

### ユーザーストーリー: カジュアルユーザーとしてのクラシックモード利用
**シナリオ:** 個人ユーザーが日常的にSNSを閲覧
1. User: デフォルトのクラシックモードで利用
2. System: デバイスに応じた最適なレイアウト表示
3. User: タイムラインをスクロールして閲覧
4. User: 通知タブをタップして確認
5. System: シンプルで直感的なナビゲーション提供
6. User: 学習コスト無しで快適に利用

### ユーザーストーリー: マルチデバイスユーザーとしての設定同期
**シナリオ:** PCとタブレットを使い分けるユーザー
1. User (PC): デッキモードで作業用パネル構成を作成
2. System: 設定をクラウドに自動保存
3. User (Tablet): アプリケーションを開く
4. System: PCで作成した設定を自動適用
5. User (Tablet): タブレット用に一部パネルを調整
...
### 9.2. Component Library Specifications
...
## 11. API Design (Gateway経由のエンドポイント)
...
## 12. Operations & Monitoring (運用と監視)
...
### Integration Tests
- Gateway連携: GraphQL、SSEエンドポイントとの通信
...
const uploadLink = createUploadLink({
  uri: import.meta.env.VITE_API_URL || 'https://api.avion.example/graphql',
  credentials: 'include',
});
...
export const apolloClient = new ApolloClient({
  link: authLink.concat(errorLink).concat(retryLink).concat(uploadLink),
  cache,
  defaultOptions: {
    watchQuery: {
...
## 18. SSE Client Implementation

### 18.1. SSE Connection Manager
...
### 22.4. Screen Transitions and UI/UX Design
...
## 23. State Management with Zustand
...
### 24.4. Memo and Callback Optimization
...
describe('PWA Installation Flow', () => {
  ...
  test('should track installation analytics', async () => {
    ...
    vi.doMock('../../src/utils/analytics', () => mockAnalytics);
...
```json
{
  "scripts": {
    ...
    "test:all": "pnpm test:coverage && pnpm test:e2e && pnpm test:lighthouse"
  }
}
```
