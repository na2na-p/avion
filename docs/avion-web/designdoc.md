...
- **実装要件**: すべての状態変更操作に対してダブルサブミットクッキーパターンを実装し、APIルートでOriginヘッダーを検証し、SameSiteクッキー属性を使用し、GraphQLミューテーションにCSRFトークンを含めることを確実にします。BFFレイヤーは、バックエンドサービスにリクエストを転送する前にトークンを検証する必要があります。

### セキュリティヘッダー
- **ガイドライン**: [../common/security/security-headers.md](../common/security/security-headers.md)
- **実装要件**: Next.jsを設定して、X-Frame-Options、X-Content-Type-Options、Referrer-Policy、Permissions-Policyを含む適切なセキュリティヘッダーを送信するようにします。スクリプト、スタイル、接続に必要なソースのみを許可する厳格なContent Security Policyを実装します。ヘッダーは、フロントエンドアプリケーションとAPIルートの両方で設定される必要があります。
