...
| エラーコード | HTTPステータス | 説明 | 対処法 |
|------------|--------------|------|--------|
| WEB_DOMAIN_CONVERSATION_NOT_FOUND | 404 | 会話が見つかりません | 会話IDを確認してください |
| WEB_DOMAIN_MESSAGE_NOT_FOUND | 404 | メッセージが見つかりません | メッセージIDを確認してください |
| WEB_DOMAIN_MESSAGE_EDIT_EXPIRED | 403 | メッセージの編集期限が切れています | 送信後15分以内のみ編集可能です |
| WEB_DOMAIN_CONVERSATION_PARTICIPANT_LIMIT | 400 | 会話参加者数の上限に達しました | 参加者数は最大100人です |
| WEB_DOMAIN_MESSAGE_TOO_LONG | 400 | メッセージが長すぎます | 10,000文字以内にしてください |
| WEB_DOMAIN_MESSAGE_SEND_NOT_ALLOWED | 403 | メッセージ送信が許可されていません | 会話への参加権限を確認してください |

### E2E Encryption関連

| エラーコード | HTTPステータス | 説明 | 対処法 |
|------------|--------------|------|--------|
| WEB_DOMAIN_ENCRYPTION_SESSION_INVALID | 400 | 暗号化セッションが無効です | セッションを再確立してください |
| WEB_DOMAIN_ENCRYPTION_KEY_EXPIRED | 401 | 暗号化鍵が期限切れです | 鍵のローテーションを実行してください |
| WEB_DOMAIN_DECRYPTION_FAILED | 500 | メッセージの復号に失敗しました | 暗号化鍵を確認してください |
| WEB_DOMAIN_KEY_VERIFICATION_FAILED | 400 | 鍵の検証に失敗しました | 相手の鍵フィンガープリントを確認してください |
| WEB_DOMAIN_KEY_STORAGE_ERROR | 500 | 暗号化鍵の保存に失敗しました | ブラウザのIndexedDBを確認してください |
| WEB_DOMAIN_KEY_EXCHANGE_FAILED | 500 | 鍵交換に失敗しました | ネットワーク接続を確認して再試行してください |
...
| WEB_USECASE_WEBSOCKET_RECONNECT_EXHAUSTED | 503 | WebSocket再接続の試行回数を超過しました | ページを再読み込みしてください |
| WEB_USECASE_WEBSOCKET_AUTH_FAILED | 401 | WebSocket認証に失敗しました | 再ログインしてください |
...
| エラーコード | HTTPステータス | 説明 | 対処法 |
|------------|--------------|------|--------|
| WEB_USECASE_MESSAGE_SEND_FAILED | 500 | メッセージ送信に失敗しました | 再試行してください |
| WEB_USECASE_MESSAGE_DELIVERY_TIMEOUT | 504 | メッセージ配信がタイムアウトしました | ネットワーク接続を確認してください |
| WEB_USECASE_CONVERSATION_CREATE_FAILED | 500 | 会話の作成に失敗しました | 再試行してください |
| WEB_USECASE_CONVERSATION_LOAD_FAILED | 500 | 会話の読み込みに失敗しました | 再試行してください |
| WEB_USECASE_MESSAGE_HISTORY_LOAD_FAILED | 500 | メッセージ履歴の読み込みに失敗しました | 再試行してください |
| WEB_USECASE_TYPING_INDICATOR_FAILED | 500 | タイピングインジケーターの送信に失敗しました | 無視して問題ありません |
| WEB_USECASE_READ_RECEIPT_FAILED | 500 | 既読通知の送信に失敗しました | 自動的に再試行されます |
| WEB_USECASE_MESSAGE_SEARCH_FAILED | 500 | メッセージ検索に失敗しました | 検索条件を確認して再試行してください |
| WEB_USECASE_ENCRYPTION_INIT_FAILED | 500 | 暗号化セッションの初期化に失敗しました | ページを再読み込みして再試行してください |
| WEB_USECASE_KEY_EXCHANGE_FAILED | 500 | 鍵交換に失敗しました | 相手がオンラインの時に再試行してください |
| WEB_USECASE_MESSAGE_BUFFER_OVERFLOW | 507 | メッセージバッファが容量超過しました | 接続を再確立してください |
...
| エラーコード | HTTPステータス | 説明 | 対処法 |
|------------|--------------|------|--------|
| WEB_INFRA_WEBSOCKET_PROTOCOL_ERROR | 400 | WebSocketプロトコルエラー | ブラウザの互換性を確認してください |
| WEB_INFRA_WEBSOCKET_CLOSE_ABNORMAL | 500 | WebSocket異常切断 | 自動再接続を待つか、ページを再読み込みしてください |
| WEB_INFRA_WEBSOCKET_MESSAGE_PARSE_ERROR | 400 | WebSocketメッセージ解析エラー | メッセージ形式を確認してください |
| WEB_INFRA_WEBSOCKET_HEARTBEAT_TIMEOUT | 504 | WebSocketハートビートタイムアウト | 接続が自動的に再確立されます |

### Encryption Storage関連

| エラーコード | HTTPステータス | 説明 | 対処法 |
|------------|--------------|------|--------|
| WEB_INFRA_INDEXEDDB_KEY_STORE_ERROR | 500 | 暗号化鍵ストアエラー | ブラウザのIndexedDBを確認してください |
| WEB_INFRA_CRYPTO_API_NOT_SUPPORTED | 400 | Web Crypto APIがサポートされていません | 最新のブラウザを使用してください |
| WEB_INFRA_KEY_DERIVATION_FAILED | 500 | 鍵導出に失敗しました | ブラウザを再起動してください |
| WEB_INFRA_SECURE_RANDOM_FAILED | 500 | セキュアな乱数生成に失敗しました | ブラウザを再起動してください |
