...
*   シャドウバン（スパム対策）
*   制限付きリーチ（センシティブコンテンツ）
*   **警告通知の発行**: モデレーションアクション（警告、コンテンツ削除、アカウント制限等）の実行時に `moderation_action_taken` イベントをNATS JetStreamに発行する。`avion-notification` が当該イベントを購読し、対象ユーザーへの通知配信を担当する。本サービスは通知の直接配信を行わず、イベント発行のみを責務とする
...
#### インスタンスレベル制御
*   インスタンスブロック（フェデレーション制御）。インスタンスブロックのポリシー判定は本サービスが担当し、ActivityPubプロトコル上の技術的な遮断実行はavion-activitypubが担当する。ポリシー変更時はNATS JetStreamイベント（`moderation.instance_policy.changed`）を発行し、avion-activitypubが同期する
*   ドメインレベルのメディアプロキシ制限
*   リモートユーザーの自動サスペンド設定
*   インスタンスレピュテーション管理
*   フェデレーション通報の処理
...
*   **直接的な通知配信**: `avion-notification`が担当。本サービスはモデレーションアクション実行時に `moderation_action_taken` イベントを発行するのみであり、ユーザーへの通知生成・SSE/Web Push配信は `avion-notification` が一元的に担当する
...
*   **コンテンツフィルタリング（非同期イベント検査）**: `avion-drop` は `DropCreatedEvent` を発行し、本サービスが非同期で検査する。違反検出時は本サービスが事後削除・制限アクションを実行する。`avion-drop` の `DROP_USECASE_CONTENT_FILTERING_FAILED` エラーは廃止する

#### 非同期コンテンツ検査のタイミングギャップ

avion-dropが非同期イベント検査方式を採用しているため、Dropが作成されてからavion-moderationの検査が完了するまでの間、コンテンツはユーザーに表示される可能性がある。この「タイミングギャップ」への対策:

- **SLO**: Drop作成からモデレーション検査完了まで5秒以内を目標とする
- **ハイリスクコンテンツ**: NGワードフィルターはavion-drop側の同期処理として維持（明確な違反パターンのみ。詳細は [avion-drop PRD](../avion-drop/prd.md) を参照）
- **検査完了前の表示**: コンテンツは通常通り表示される。検査結果により違反が判明した場合はavion-moderationが事後的にアクションを実行する
- **アクション実行**: 違反検出時はavion-moderationがイベント経由で即座に非表示/削除処理を実行し、avion-notificationを通じて投稿者に通知する
*   **モデレーション階層構造（moderation優先）**: 評価順序: (1) 本サービスのプラットフォーム全体NGワード → ブロック (2) `avion-community` の `auto_moderation_rules`（コミュニティローカルのキーワードフィルタ）→ コミュニティ内のみ適用。プラットフォームポリシーがコミュニティルールを常に上回る。コミュニティオーナーはプラットフォームポリシーを緩和できない
*   **NSFW/センシティブ管理（3層構造・最終決定権）**: 本サービスはプラットフォームポリシーに基づく強制適用を担当し、最終決定権を持つ。`avion-media` のNSFWフラグおよび `avion-drop` のユーザー自己申告を上書き可能。優先度順: 本サービス強制適用 > media ML検出 > ユーザー自己申告。NSFW検出からモデレーション連携までの詳細フローは [avion-media PRD](../avion-media/prd.md) の「NSFW検出・モデレーション連携フロー」を参照
*   **監査ログ（分離維持 + 統合検索API）**: 本サービスのモデレーション監査ログ（ハッシュチェーン保護付き）と `avion-system-admin` の管理者操作監査ログはそれぞれのサービスで保持を維持する。`avion-system-admin` が統合検索APIを提供し、両系統のログを横断検索可能にする

### コンテンツモデレーション vs プライバシー制御の責務区分

本サービスが扱う「コンテンツモデレーション」と、`avion-user` が扱う「プライバシー制御」は明確に区分する。

| 観点 | avion-moderation（コンテンツモデレーション） | avion-user（プライバシー制御） |
|:--|:--|:--|
| **目的** | プラットフォームの健全性維持・ガイドライン違反の検出と対処 | 個人のプライバシー保護・対人関係制御 |
| **判断主体** | プラットフォーム（モデレーター、AI、コミュニティ投票） | ユーザー本人 |
| **対象** | コンテンツの内容そのもの（スパム、ヘイトスピーチ、違法コンテンツ等） | ユーザー間の関係性（ブロック、ミュート等） |
| **影響範囲** | プラットフォーム全体（全ユーザーに対してコンテンツ非表示・削除） | 個人スコープ（設定したユーザー本人にのみ適用） |
| **具体的な機能** | 通報処理、コンテンツフィルタリング、モデレーションアクション（警告・削除・凍結）、異議申し立て | ブロック、ミュート、フォロー承認制、公開範囲設定 |
| **データ保持** | 監査ログとして長期保持（法的要件：最大7年） | ユーザー設定として保持（ユーザー削除時に削除） |

**重要な原則:**
*   `avion-user` のブロック/ミュート設定は個人の表示制御であり、プラットフォーム全体のコンテンツ可視性には影響しない
*   `avion-moderation` の判定（コンテンツ削除、アカウント凍結等）はプラットフォーム全体に適用され、個人設定に関係なく強制される
*   あるユーザーがブロックしている相手のコンテンツが通報された場合、通報処理は本サービスが担当するが、ブロック状態の確認は `avion-user` に問い合わせる

### 管理機能の責務範囲

本サービスが持つ管理機能は、コンテンツモデレーションに固有のものに限定する。システム横断的な管理機能は `avion-system-admin` が担当する。

| 管理機能 | 担当サービス | 補足 |
|:--|:--|:--|
| 通報処理・レビュー | avion-moderation | 通報キュー管理、モデレーターへの割り当て |
| コンテンツ非表示・削除 | avion-moderation | 違反コンテンツの事後削除・制限 |
| アカウント制限（一時停止・永久凍結） | avion-moderation | モデレーション判定に基づく制裁 |
| コンテンツフィルタリングルール管理 | avion-moderation | NGワード、正規表現、MLモデル設定 |
| モデレーション監査ログ | avion-moderation | ハッシュチェーン保護付き監査ログ |
| モデレーターダッシュボード・統計 | avion-moderation | モデレーション固有の統計・KPI |
| 異議申し立て処理 | avion-moderation | 異議申し立ての受付・再審査 |
| インスタンスポリシー管理 | avion-moderation | フェデレーション先のブロック・サイレンス |
| システム設定・グローバル設定 | avion-system-admin | レート制限、機能フラグ等 |
| アナウンス・メンテナンス通知 | avion-system-admin | プラットフォーム全体への通知 |
| バックアップ・リストア | avion-system-admin | システム全体のデータ保全 |
| 管理者操作監査ログ | avion-system-admin | システム管理操作の監査記録 |
| 統合監査ログ検索 | avion-system-admin | 複数サービスの監査ログ横断検索API |
| ユーザー認証・認可 | avion-auth | JWT発行、ロール管理 |
