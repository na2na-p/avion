...
```go
var (
	ErrReportDuplicate          = NewDomainError("MOD_DOMAIN_REPORT_DUPLICATE", "24時間以内の同一対象への重複通報")
	ErrReportNotPending         = NewDomainError("MOD_DOMAIN_REPORT_NOT_PENDING", "通報は処理待ち状態ではありません")
	ErrAppealDeadlineExpired    = NewDomainError("MOD_DOMAIN_APPEAL_EXPIRED", "異議申し立て期限が過ぎています")
	ErrModerationUnauthorized   = NewDomainError("MOD_USECASE_UNAUTHORIZED", "モデレーション権限が不足しています")
	ErrActionNotReversible      = NewDomainError("MOD_DOMAIN_ACTION_NOT_REVERSIBLE", "このアクションは取り消しできません")
	ErrActionAlreadyActive      = NewDomainError("MOD_DOMAIN_ACTION_ALREADY_ACTIVE", "モデレーションアクションは既に有効です")
	ErrFilterInactive           = NewDomainError("MOD_DOMAIN_FILTER_INACTIVE", "フィルターが無効化されています")
	ErrFilterPriorityConflict   = NewDomainError("MOD_DOMAIN_FILTER_PRIORITY_CONFLICT", "フィルター優先度が競合しています")
	ErrInstanceSelfModeration   = NewDomainError("MOD_DOMAIN_INSTANCE_SELF_MODERATION", "自インスタンスはモデレーション対象外です")
	ErrEscalationRequired       = NewDomainError("MOD_DOMAIN_ESCALATION_REQUIRED", "上級モデレーターへのエスカレーションが必要です")
	ErrOptimisticLockFailure    = NewDomainError("MOD_DOMAIN_OPTIMISTIC_LOCK_FAILURE", "楽観的ロックの競合が発生しました")
)
...
```go
var (
	ErrMLClassifierUnavailable = NewInfraError("MOD_INFRA_ML_CLASSIFIER_UNAVAILABLE", "機械学習分類器が利用できません", true)
	ErrImageModerationTimeout  = NewInfraError("MOD_INFRA_IMAGE_MODERATION_TIMEOUT", "画像解析がタイムアウトしました", true)
	ErrAuditLogCorruption      = NewInfraError("MOD_INFRA_AUDIT_LOG_CORRUPTION", "監査ログの改竄が検出されました", false)
	ErrQueueServiceDown        = NewInfraError("MOD_INFRA_QUEUE_SERVICE_DOWN", "モデレーションキューサービスが停止しています", true)
	ErrDatabaseConnection      = NewInfraError("MOD_INFRA_DB_CONNECTION_ERROR", "データベース接続エラー", true)
	ErrRedisConnectionLost     = NewInfraError("MOD_INFRA_REDIS_CONNECTION_LOST", "Redisサーバーとの接続が切断されました", true)
	ErrEventPublishFailed      = NewInfraError("MOD_INFRA_EVENT_PUBLISH_FAILED", "イベント発行に失敗しました", true)
)
```
...
- **フロー 6: インスタンスポリシー設定 (Command)**
  ...
  5. InstancePolicyEventPublisher: `avion.moderation.instance.policy_applied` イベント発行
...
// avion.moderation.instance.policy_applied
{
  "event_type": "avion.moderation.instance.policy_applied",
  "domain": "example.com",
...
### 19.3. Audit Trail Integrity Testing (監査証跡整合性テスト)
