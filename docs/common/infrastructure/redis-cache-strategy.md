...
| **avion-activitypub** | - | Actor 情報（例外TTL: 1時間） | 公開鍵キャッシュ（例外TTL: 6時間） |

### 1.3 例外 TTL

標準の 3 段階分類（Hot: 5分、Warm: 1時間、Cold: 24時間）に当てはまらないキャッシュデータが存在します。これらはセキュリティ要件やプロトコル要件により、個別の TTL 設定が必要です。

| データ | TTL | 理由 | サービス |
|:--|:--|:--|:--|
| **ActivityPub 公開鍵** | 6時間 | セキュリティ要件による中間値。鍵のローテーション頻度（通常数日〜数週間）に対して十分に短く、かつ頻繁なリモートサーバーへの鍵取得リクエストを抑制するバランス値。Hot（5分）では取得頻度が過剰、Cold（24時間）では鍵失効時の反映が遅すぎる | avion-activitypub |
| **ActivityPub Actor 情報** | 1時間 | Warm 相当の TTL だが、ActivityPub プロトコル要件として固定。リモート Actor のプロフィール情報は比較的頻繁に参照されるが、更新頻度は低い。プロトコル仕様で推奨される更新間隔に準拠 | avion-activitypub |
| **JWT 検証結果キャッシュ** | 1分 | セキュリティ即時性要件。JWT の失効（revoke）が発生した場合、最大 1 分以内にキャッシュが無効化される必要がある。Hot（5分）では失効した JWT による不正アクセスのリスク窓が大きすぎる | avion-auth |
...
const (
    // TTLAPPublicKey は ActivityPub 公開鍵のキャッシュ TTL
    // セキュリティ要件により Hot/Warm/Cold の中間値を採用
    TTLAPPublicKey = 6 * time.Hour

    // TTLAPActorInfo は ActivityPub Actor 情報のキャッシュ TTL
    // プロトコル要件により Warm 相当の固定値を採用
    TTLAPActorInfo = 1 * time.Hour

    // TTLJWTVerification は JWT 検証結果のキャッシュ TTL
    // セキュリティ即時性要件により極短 TTL を採用
    TTLJWTVerification = 1 * time.Minute
)
```

#### キャッシュキーパターン（例外 TTL 対象）

| キーパターン | TTL | 説明 |
|:--|:--|:--|
| `activitypub:pubkey:{actor_uri_hash}` | 6時間 | リモート Actor の公開鍵 |
| `activitypub:actor:{actor_uri_hash}` | 1時間 | リモート Actor 情報 |
| `auth:jwt-verify:{token_hash}` | 1分 | JWT 検証結果キャッシュ |
...
| **avion-activitypub** | `activitypub:pubkey:{actor_uri_hash}` | 6時間 | 例外 | Actor 情報更新イベント | String |
| | `activitypub:actor:{actor_uri_hash}` | 1時間 | 例外 | Actor 情報更新イベント | Hash |
...
| avion-activitypub | 30,000 | 1KB | 30MB |
| **合計** | | | **約 1,000MB** |
