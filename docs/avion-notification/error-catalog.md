...
**Error Code Prefix:** `NOTIFICATION_`
...
| NOTIFICATION_DOMAIN_TEMPLATE_VARIABLE_MISSING | 400 | INVALID_ARGUMENT | テンプレート変数が不足しています | 必要な変数を確認してください。テンプレート変数は最大20個まで定義可能です |
...
| NOTIFICATION_DOMAIN_CHANNEL_RATE_LIMITED | 429 | RESOURCE_EXHAUSTED | チャンネルレート制限中です | 制限がリセットされるまで待ってください。Push通知: 1000件/分/ユーザー、メール: 100件/時/ユーザー |
...
| エラーコード | HTTPステータス | gRPCステータス | 説明 | 対処法 |
|------------|--------------|--------------|------|--------|
| NOTIFICATION_DOMAIN_GROUP_NOT_FOUND | 404 | NOT_FOUND | 通知グループが見つかりません | グループIDを確認してください |
| NOTIFICATION_DOMAIN_INVALID_GROUP_TYPE | 400 | INVALID_ARGUMENT | グループ種別が不正です | グループ化可能な通知種別を確認してください |
| NOTIFICATION_DOMAIN_CANNOT_GROUP_DIFFERENT_TYPES | 400 | INVALID_ARGUMENT | 異なる種別の通知はグループ化できません | 同一種別の通知を指定してください |
| NOTIFICATION_DOMAIN_GROUP_LIMIT_EXCEEDED | 429 | RESOURCE_EXHAUSTED | グループ上限を超過しました | グループ内の通知数を確認してください（1グループあたり最大100件） |

### Announcement関連

| エラーコード | HTTPステータス | gRPCステータス | 説明 | 対処法 |
|------------|--------------|--------------|------|--------|
| NOTIFICATION_DOMAIN_ANNOUNCEMENT_NOT_FOUND | 404 | NOT_FOUND | アナウンスが見つかりません | アナウンスIDを確認してください |
| NOTIFICATION_DOMAIN_ANNOUNCEMENT_EXPIRED | 410 | NOT_FOUND | アナウンスが期限切れです | 新しいアナウンスを作成してください |
| NOTIFICATION_DOMAIN_INVALID_ANNOUNCEMENT_PERIOD | 400 | INVALID_ARGUMENT | アナウンス期間が不正です | 開始/終了日時を確認してください |
| NOTIFICATION_DOMAIN_UNAUTHORIZED_ANNOUNCEMENT | 403 | PERMISSION_DENIED | アナウンス作成権限がありません | 管理者権限を確認してください |

### SSE接続関連

| エラーコード | HTTPステータス | gRPCステータス | 説明 | 対処法 |
|------------|--------------|--------------|------|--------|
| NOTIFICATION_DOMAIN_SSE_CONNECTION_LIMIT_EXCEEDED | 429 | RESOURCE_EXHAUSTED | SSE接続数の上限に達しました | 不要な接続を閉じてから再接続してください。同一ユーザーの接続数上限、または同一IPからの接続数上限（10接続）を超過しています |

### メッセージ通知関連

| エラーコード | HTTPステータス | gRPCステータス | 説明 | 対処法 |
|------------|--------------|--------------|------|--------|
| NOTIFICATION_DOMAIN_MESSAGE_EVENT_INVALID | 400 | INVALID_ARGUMENT | メッセージイベントが不正です | イベントペイロードの形式を確認してください |
| NOTIFICATION_DOMAIN_MESSAGE_RECIPIENT_MUTED | 200 | OK | 受信者がメッセージ通知をミュートしています | ミュート設定を確認してください（正常応答） |
| NOTIFICATION_DOMAIN_CONVERSATION_NOT_FOUND | 404 | NOT_FOUND | 会話が見つかりません | 会話IDを確認してください |
| NOTIFICATION_DOMAIN_DM_SETTING_CONFLICT | 409 | ALREADY_EXISTS | DM通知設定の競合が発生しました | avion-notificationのグローバル設定とavion-messageの会話単位設定の間で競合が検出されました。グローバル設定を優先し、会話単位設定を再確認してください |
...
| NOTIFICATION_USECASE_RATE_LIMIT_EXCEEDED | 429 | RESOURCE_EXHAUSTED | レート制限を超過しました | しばらく待ってから再試行してください。ユーザー: 1000req/min、管理者: 10000req/min |
...
| NOTIFICATION_USECASE_BATCH_DELIVERY_FAILED | 502 | INTERNAL | バッチ配信に失敗しました | バッチ設定を確認してください（1バッチあたり最大100件） |
...
| エラーコード | HTTPステータス | gRPCステータス | 説明 | 対処法 |
|------------|--------------|--------------|------|--------|
| NOTIFICATION_INFRA_VAPID_KEY_INVALID | 500 | INTERNAL | Web Push用VAPIDキーが無効または破損しています | 環境変数に設定されたVAPIDキーペア（公開鍵・秘密鍵）の形式と有効性を確認してください。キーの再生成が必要な場合があります |
...
> **注記:** avion-notification はリアルタイム配信に SSE を使用します。WebSocket はスコープ外です（DesignDoc Non-Goals 参照）。WebSocket 関連エラーは avion-message が担当します。

| エラーコード | HTTPステータス | gRPCステータス | 説明 | 対処法 |
|------------|--------------|--------------|------|--------|
| NOTIFICATION_HANDLER_BAD_REQUEST | 400 | INVALID_ARGUMENT | 不正なリクエスト | リクエスト形式を確認してください |
| NOTIFICATION_HANDLER_PAYLOAD_TOO_LARGE | 413 | INVALID_ARGUMENT | ペイロードサイズ超過 | リクエストサイズを小さくしてください（通知データ: 最大4KB、Pushペイロード: 最大4KB） |
| NOTIFICATION_HANDLER_UNSUPPORTED_MEDIA_TYPE | 415 | INVALID_ARGUMENT | サポートされないメディアタイプ | Content-Typeを確認してください |
| NOTIFICATION_HANDLER_RATE_LIMIT_EXCEEDED | 429 | RESOURCE_EXHAUSTED | レート制限を超過しました | しばらく待ってから再試行してください。ユーザー: 1000req/min、管理者: 10000req/min |
