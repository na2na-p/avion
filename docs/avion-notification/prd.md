...
## avion-message との通知責務境界

avion-notification はプラットフォーム全体の通知生成・配信を一元管理する。ダイレクトメッセージ（DM）に関連する通知についても、以下の責務分担に従う。

### avion-notification の責務
- **DM関連通知の生成・配信**: 新規メッセージ受信、会話への招待、メッセージリアクション等の通知を生成し、SSE/Web Push で配信する
- **通知設定の管理**: DM関連通知の有効/無効設定を含む、全通知種別の設定を一元管理する
- **通知の永続化・既読管理**: DM関連通知を他の通知と同様に管理する

### avion-message の責務
- **メッセージ固有の通知設定**: 会話単位のミュート設定、Do Not Disturb (DnD) 設定、メンション通知の優先度設定など、メッセージングに固有の通知制御を管理する
- **イベントの発行**: 新規メッセージ送信時に `message.sent` イベント、メッセージ配信完了時に `message.delivered` イベント等を NATS JetStream に発行する
- **リアルタイム配信の制御**: WebSocket 経由のメッセージ本文のリアルタイム配信は avion-message が直接担当する（avion-notification は経由しない）

### 連携フロー
1. avion-message がメッセージ送受信等のビジネスロジックを処理し、`message.sent` 等のドメインイベントを NATS JetStream に発行する
2. avion-notification が当該イベントを消費し、受信者の通知設定（avion-notification 管理）とメッセージ固有設定（avion-message 管理、イベントペイロードに含む）を考慮して通知を生成・配信する
3. メッセージ本文のリアルタイム配信（WebSocket）は avion-message が直接行い、avion-notification はプッシュ通知バッジ更新やSSE通知のみを担当する

### 通知設定の優先度解決ルール

通知設定が競合する場合、以下の優先度で解決する:

1. **avion-notification のグローバル設定（最優先）**: ユーザーが通知種別を全体OFFにした場合、avion-message の個別設定に関わらず通知は配信されない
2. **avion-message の会話単位設定**: グローバル設定がONの場合のみ有効。会話ミュート、Do Not Disturb設定により通知を抑制可能
3. **デフォルト設定**: 上記いずれも未設定の場合は通知ON

この優先度ルールにより、ユーザーは avion-notification で一括制御し、avion-message で細粒度の制御を行うことができる。
...
*   **スケーラブル処理**: 毎秒10,000件の通知生成、同時SSE接続100,000セッションの処理能力。イベント処理遅延を平均2秒以内に維持。
...
*   ダイレクトメッセージ関連の通知データの生成（`avion-message` が発行する `message.sent`, `message.delivered` 等のイベントを消費）。
...
*   モデレーション関連通知（モデレーターからの警告、関係切断通知）。`avion-moderation` が発行する `moderation_action_taken` イベントをNATS JetStreamで購読し、対象ユーザーへの警告通知・コンテンツ削除通知・アカウント制限通知等を生成・配信する。
*   サーバーアナウンス配信機能。`avion-system-admin` がアナウンスコンテンツを生成し、本サービスが配信インフラとしてSSE/Web Push/メール等のチャネルを通じてユーザーへ配信する。アナウンスの作成・管理は `avion-system-admin` の責務であり、本サービスは配信と既読管理のみを担当する。
...
*   **イベント発生元のビジネスロジック:** フォロー処理自体 (`avion-user`) やリアクション処理自体 (`avion-drop`)、メッセージ送受信自体 (`avion-message`) は担当しない。イベントを受け取って通知を生成するのみ。
*   **メッセージ本文のリアルタイム配信:** WebSocket経由のメッセージ配信は `avion-message` が直接担当する。本サービスはプッシュ通知バッジ更新とSSE通知のみ担当する。
*   **メッセージ固有の通知制御:** 会話単位のミュート設定やDnD設定は `avion-message` が管理する。本サービスは `avion-message` がイベントペイロードに含めた設定情報を参照するのみ。
...
**識別子関連**
- **NotificationID**: 通知の一意識別子（UUID v7）
...
- **NotificationType**: 通知種別を表す列挙型
  ...
  - `direct_message`: ダイレクトメッセージ受信通知
  - `group_message`: グループメッセージ受信通知
  - `message_reaction`: メッセージリアクション通知
  - `conversation_invite`: 会話招待通知
- **EventType**: イベント種別を表す列挙型
  ...
  - `message_sent`, `message_delivered`, `message_reaction_added`, `conversation_created`, `participant_added`
...
*   **Avion マイクロサービス**: イベント発行元として通知をトリガーする（avion-user、avion-drop、avion-message、avion-activitypub、avion-moderation等）
...
2.  `avion-drop` が EventType='reaction_created' の DomainEvent を NATS JetStream に発行
3.  NotificationEventHandler が NATS JetStream からイベントを取得
...
2.  `avion-drop` が EventType='poll_voted' の DomainEvent を NATS JetStream に発行
...
1.  モデレーターがユーザーに対して警告アクションを実行（`avion-moderation`が処理）
2.  `avion-moderation` が EventType='moderation_action_taken' のイベントをNATS JetStreamに発行（アクション種別: warn, delete_content, suspend_account, ban_account等を含む）
3.  NotificationEventHandler がNATS JetStreamからイベントを購読・取得
4.  対象ユーザーにアクション種別に応じた通知を生成（moderation_warning、severed_relationships等）
5.  通知内容に警告理由、対処方法、異議申し立て手順（該当する場合）を含める
6.  SSE/Web Push経由で即座に配信
...
*   **通知生成**: 22種類の通知タイプに対応し、各通知の優先度・配信チャネル・グループ化ルールを適切に適用すること
...
*   **処理能力**: 毎秒10,000通知生成、同時SSE接続100,000セッション、Web Push送信5,000件/秒
*   **データベース**: 読み取りレプリカによる負荷分散、パーティショニング（ユーザーID、作成日時）、インデックス最適化
*   **水平スケーリング**: イベント処理ワーカーの動的スケーリング、NATS JetStream、メッセージキューのパーティション分散
...
*   **テスト**: ユニットテスト85%以上、統合テスト、E2Eテスト、カオスエンジニアリング、パフォーマンステスト
...
| Phase | 内容 | 期間 | 前提条件 |
|:------|:-----|:-----|:---------|
| Phase 1 | Core Notification | Week 1-2 | PostgreSQL 17/Redis 8+/NATS環境構築 |
| Phase 2 | Realtime Delivery (SSE) | Week 3-4 | Phase 1 完了 |
...
*   **Phase 1**: Notification/NotificationEvent Aggregateの実装、PostgreSQLスキーマ作成（UUID v7によるID生成）、NATS JetStreamイベント購読基盤、基本的な通知CRUD API。リリース基準: カバレッジ85%以上。
*   **Phase 2**: SSEConnectionManager Aggregate実装、リアルタイム通知配信（heartbeat 15秒間隔）、同時接続100,000セッション対応。リリース基準: SSE配信レイテンシp99 < 500ms。
*   **Phase 3**: WebPushSubscription Aggregate（RFC 8291準拠暗号化）、NotificationPreference Aggregate、通知設定管理API。リリース基準: Web Push送信成功率95%以上。
*   **Phase 4**: 通知グループ化、特定ユーザー投稿通知、モデレーション/管理者通知、サーバーアナウンス配信。
*   **Phase 5**: パフォーマンス最適化、負荷テスト（毎秒10,000通知、SSE 100,000接続）、カオスエンジニアリング。リリース基準: 稼働率99.9%達成。
...
詳細は[Design Doc: Release Plan](./designdoc.md#17-release-plan-リリース計画)を参照。
