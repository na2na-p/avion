...
*   ユーザー設定管理
    ...
    - アクセシビリティ設定
*   ユーザー検索
    - ユーザー名による完全一致検索（avion-user内で実行）
    - 表示名による部分一致検索（avion-user内で実行）
    - プロフィール全文検索（avion-search に gRPC 呼び出しで委譲。avion-user はプライバシーフィルタリングとブロック/ミュートフィルタリングを適用した上で結果を返却する）
    - おすすめユーザーの提案
*   アカウント状態管理
    - アクティブ/非アクティブ状態
    - 凍結（サスペンド）状態
    ...
    - メール確認済みフラグ
    - 認証済みバッジ
    - **ユーザー削除カスケード:** ユーザーがアカウント削除を要求した場合、avion-user が Saga パターンのオーケストレーターとして機能する。30日間の猶予期間（論理削除）の後、`avion.user.account.permanently_deleted` イベントを NATS JetStream に発行し、各サービス（avion-auth, avion-drop, avion-timeline, avion-message, avion-notification, avion-media, avion-search, avion-community, avion-moderation, avion-activitypub）が自身のデータを削除・匿名化する。avion-user は全サービスからの `deletion_completed` イベントを集約し、すべての削除が完了したことを確認した上で最終的にユーザー基本レコードを削除する（最終的な整合性）。詳細は [ユーザー削除カスケード設計](../common/architecture/user-deletion-cascade.md) を参照。
*   リスト機能
    - ユーザーリストの作成・管理
    - リストメンバーの追加・削除
    - 公開/非公開リスト設定
...
*   インポート/エクスポート機能
    ...
    - 他のインスタンスからのデータインポート
    - アカウント移行サポート
*   統計情報（UserStats カウンター集約）
    - 投稿数カウント（avion-dropと連携）
    - フォロー/フォロワー数
    - 獲得したリアクション数の集計
    - アカウント作成からの経過日数
    - **更新方式:** avion-user が各サービスからのドメインイベント（`drop.created`, `drop.deleted`, `reaction.created` 等）を NATS JetStream 経由で消費し、UserStats カウンターを非同期で更新する。フォロー/フォロワー数はフォロー操作時にトランザクション内で同期的に更新する。定期的な再計算バッチ（毎時実行）により、イベント欠損時の整合性を保証する。
*   ActivityPub連携用メタデータ
    - Actor情報の提供
    - publicKeyの管理
    - inboxとoutboxのURL管理
...
*   **DM（ダイレクトメッセージ）機能:** `avion-message` サービスが担当。avion-userはDM送信可否の判定に必要なプライバシー設定（`dm_from_followers_only` 等）のマスターデータを管理し、gRPC APIで提供する。
...
**識別子関連**
- **UserID**: ユーザーの一意識別子（UUID v7、128bit、時系列ソート可能）
- **Username**: ユーザー名（3-30文字、英数字とアンダースコア、大文字小文字を区別しない一意性）
- **Email**: メールアドレス（RFC 5322準拠、国際化ドメイン名対応、最大254文字）
- **FollowID**: フォロー関係の識別子（UUID v7、128bit、分散環境での衝突回避、時系列ソート可能）
- **BlockID**: ブロック関係の識別子（UUID v7、128bit、時系列ソート可能）
- **MuteID**: ミュート設定の識別子（UUID v7、128bit、設定の可逆性を保証）
- **ListID**: ユーザーリストの識別子（UUID v7、128bit、時系列ソート可能）
...
- **ActorType**: Actorタイプ（ActivityPub仕様準拠）
  ...
  - `Group`: グループアカウント（avion-communityサービスと連携。avion-communityがグループの作成・管理・メンバーシップ・モデレーションを担当し、avion-userはGroup ActorTypeのユーザーレコードとActivityPub Actor情報を管理する。グループ作成時にavion-communityからのイベントを受信してGroup ActorTypeのユーザーを自動生成する）
...
- **FollowingCount**: フォロー中数（非正規化キャッシュ値、上限なし）
...
- **メソッド**:
  ...
  - `generateSecureUserID()`: セキュアなユーザーID生成（UUID v7アルゴリズム）
...
4.  avion-authで認証情報を作成後、`avion-user` にユーザー作成イベントを送信（NATS JetStream）
...
13. UserEventPublisher で `user_created` イベントを NATS JetStream に発行
...
12. UserEventPublisher で `profile_updated` イベントを NATS JetStream に発行
...
12. FollowEventPublisher で `follow_created` イベントを NATS JetStream に発行
13. (非同期) EventHandler が以下のサービスにイベントを伝播：
...
10. UserListCleanupService でリストから対象ユーザーを除外：
    - 自分のリストから対象ユーザーを削除
    - 対象ユーザーのリストから自分を削除
11. BlockEventPublisher で `block_created` イベントを NATS JetStream に発行
...
14. MuteEventPublisher で `mute_created` イベントを NATS JetStream に発行
...
13. UserListEventPublisher で `list_created` イベントを NATS JetStream に発行
...
15. UserListEventPublisher で `list_member_added` イベントを NATS JetStream に発行
...
12. SettingsEventPublisher で `settings_updated` イベントを NATS JetStream に発行
13. (非同期) EventHandler が以下のサービスにイベントを伝播：
...
### アカウント凍結（管理者機能・avion-moderation連携）

**トリガー:**
- **管理者による手動凍結:** 管理者が管理画面から直接実行
- **avion-moderationからの凍結要求:** avion-moderationがモデレーション判定（通報処理、コンテンツフィルタリング等）に基づき `suspend_account` アクションを決定した場合、gRPC API経由でavion-userに凍結を要求する

**フロー:**
1.  管理者は管理画面でユーザーを選択し「凍結」を実行、またはavion-moderationからgRPC経由で凍結要求を受信
2.  管理者操作の場合、フロントエンドは `avion-gateway` 経由で凍結リクエストを送信
3.  SuspendUserCommandUseCase がリクエストを処理
4.  AdminAuthorizationService で管理者権限を確認（avion-moderationからの要求はサービス間認証で権限確認）
5.  UserRepository から User Aggregate を取得
6.  User.suspend() で凍結処理
7.  SuspensionReason を記録（モデレーション起因の場合、モデレーションアクションIDと理由を含む）
8.  UserRepository で永続化
9.  SessionInvalidationService でセッションを無効化
10. SuspensionEventPublisher で `user_suspended` イベントを NATS JetStream に発行
11. (非同期) EventHandler が以下のサービスにイベントを伝播：
    - avion-notification: 凍結通知を送信
    - avion-moderation: 凍結完了の確認（モデレーション起因の場合）
    - avion-drop: 凍結ユーザーの投稿の非表示化
    - avion-timeline: タイムラインからの除外
12. SuspendUserResponse DTO を返却

**avion-moderationとの責務分担:**
- **avion-moderation:** 通報処理・コンテンツフィルタリング・モデレーション判定（凍結が必要かの判断）を担当
- **avion-user:** 凍結の実行（アカウント状態変更、セッション無効化、イベント発行）を担当
- モデレーション判定に基づく制裁（コンテンツ削除、アカウント凍結等）はプラットフォーム全体に適用され、個人設定に関係なく強制される
...
*   **Privacy管理:**
    ...
    *   プライバシー設定に基づくアクセス制御
    *   キーワードミュートの高速マッチング
    *   **プライバシー設定マスターデータの一元管理:** avion-user がプライバシー設定（公開/非公開、フォロー承認制、DM受信設定、検索インデックス許可等）のマスターデータを管理する。他サービス（avion-timeline, avion-search, avion-message, avion-notification 等）は avion-user の gRPC API（`PrivacyControlDomainService` 経由）に問い合わせてフィルタを適用する。設定変更時は `PrivacySettingsChangedEvent` を NATS JetStream に発行し、各サービスがキャッシュを更新する。
...
*   **プロフィール:** 表示名50文字、自己紹介500文字、カスタムフィールド4つまで
...
*   **イベント順序**: NATS JetStreamによる順序保証されたイベント配信
...
- **PrivacySettingsChangedEvent**: プライバシー設定変更時
  ...
  - 購読サービス: avion-search, avion-timeline, avion-message, avion-activitypub
...
avion-user は以下の外部サービスからのイベントを NATS JetStream 経由で購読し、自身のデータを更新する。

#### 認証サービスからのイベント
- **iam_user_registered** (avion-auth): 新規ユーザー登録時。User Aggregate の初期プロフィール・設定・統計レコードを作成する。

#### 投稿サービスからのイベント（UserStats カウンター更新用）
- **drop.created** (avion-drop): 投稿作成時。`UserStats.incrementDropCount()` を実行する。
- **drop.deleted** (avion-drop): 投稿削除時。`UserStats.decrementDropCount()` を実行する。
- **reaction.created** (avion-drop): リアクション受信時。`UserStats.incrementReactionCount()` を実行する。
- **reaction.deleted** (avion-drop): リアクション削除時。リアクション受信カウントを減算する。

#### 削除カスケード完了イベント
- **deletion_completed** (各サービス): ユーザー削除カスケード処理において、各サービスが自身のデータ削除を完了した際に発行する完了報告イベント。avion-user がオーケストレーターとして全サービスの完了を追跡する。
...
- ユーザープロフィール管理の基本機能（作成・取得・更新・削除）
- PostgreSQL 17によるデータ永続化
- Redis 8+によるキャッシュ基盤
- リリース基準: ユニットテストカバレッジ85%以上
...
- ユーザー設定管理（プライバシー、通知、UI、アクセシビリティ）
- ユーザーリスト機能
- リリース基準: プライバシー制御のE2Eテスト合格

### Phase 4: Events & Integration (Week 7-8)
- NATS JetStreamによるイベント駆動連携
- 統計情報管理
- 他サービスとの連携（avion-search、avion-timeline、avion-activitypub）
- リリース基準: イベント配信遅延 < 100ms

### Phase 5: Optimization & GA (Week 9-10)
- パフォーマンス最適化、負荷テスト
- インポート/エクスポート機能（GDPR対応）
- 本番環境へのカナリアリリース
- リリース基準: 稼働率99.9%達成、全テストカバレッジ85%以上
