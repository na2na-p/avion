...
- ユーザー作成、プロフィール更新、フォロー/アンフォロー、ブロック/ミュート時にイベントを発行 (NATS JetStream) し、他のサービスと連携する。
...
- **DM（ダイレクトメッセージ）機能:** `avion-message` サービスが担当。avion-userはDM送信可否判定に必要なプライバシー設定のマスターデータを gRPC API で提供するのみ。
...
#### 外部サービス向けAPI（avion-timeline連携）

以下のQuery APIはavion-timelineがフィルタリング処理で利用する:
- `GetMutedUsers`: ミュート中のユーザーID一覧を返却
- `GetMuteKeywords`: ミュートキーワード一覧を返却
- `GetBlockedUsers`: ブロック中のユーザーID一覧を返却
- `GetFollowers` / `GetFollowing`: フォロー関係を返却（ホームタイムライン構築用）
- `GetSettings`: ユーザー設定（プライバシー関連フィールドを含む）を返却
...
- **Event実装:**
  - NATSEventPublisher: NATS JetStreamを使用したイベント発行
...
- **不変条件**:
  ...
  - DisplayNameは最大50文字（絵文字対応、Unicode正規化必須）
...
- **不変条件**:
  ...
  - テーマ設定は列挙値のみ（light, dark, auto, high_contrast）
...
```sql
CREATE TABLE users (
    user_id UUID PRIMARY KEY, -- UUID v7 (Backend採番)
...
CREATE TABLE profile_fields (
    field_id UUID PRIMARY KEY, -- UUID v7 (Backend採番)
...
CREATE INDEX idx_profile_fields_user_id ON profile_fields (user_id);
```

##### follows テーブル
```sql
CREATE TABLE follows (
    follow_id UUID PRIMARY KEY, -- UUID v7 (Backend採番)
...
CREATE INDEX idx_follows_is_approved ON follows (is_approved);
...
CREATE TABLE follow_requests (
    request_id UUID PRIMARY KEY, -- UUID v7 (Backend採番)
...
CREATE INDEX idx_follow_requests_requested_at ON follow_requests (requested_at);
```

##### blocks テーブル
```sql
CREATE TABLE blocks (
    block_id UUID PRIMARY KEY, -- UUID v7 (Backend採番)
...
```sql
CREATE TABLE mutes (
    mute_id UUID PRIMARY KEY, -- UUID v7 (Backend採番)
    muter_id UUID NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    muted_id UUID REFERENCES users(user_id) ON DELETE CASCADE,
...
CREATE INDEX idx_mutes_mute_type ON mutes (mute_type);
...
CREATE TABLE mute_keywords (
    keyword_id UUID PRIMARY KEY, -- UUID v7 (Backend採番)
    user_id UUID NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
...
CREATE INDEX idx_mute_keywords_user_id ON mute_keywords (user_id);
```
...
##### avion.user.user.created
```json
{
  "event_id": "evt_xxx",
  "event_type": "avion.user.user.created",
  "user_id": "123456789",
  "username": "alice",
...
##### avion.user.profile.updated
```json
{
  "event_id": "evt_xxx",
  "event_type": "avion.user.profile.updated",
  "user_id": "123456789",
  "changes": {
    "display_name": "Alice",
    "bio": "Updated bio",
...
##### avion.user.follow.created
```json
{
  "event_id": "evt_xxx",
  "event_type": "avion.user.follow.created",
...
##### avion.user.follow.deleted
```json
{
  "event_id": "evt_xxx",
  "event_type": "avion.user.follow.deleted",
  "follower_id": "123456789",
  "followee_id": "987654321",
...
##### avion.user.block.created
```json
{
  "event_id": "evt_xxx",
  "event_type": "avion.user.block.created",
...
##### avion.user.mute.created
```json
{
  "event_id": "evt_xxx",
  "event_type": "avion.user.mute.created",
  "muter_id": "123456789",
  "muted_id": "987654321",
  "mute_type": "user",
...
##### avion.user.user.suspended
```json
{
  "event_id": "evt_xxx",
  "event_type": "avion.user.user.suspended",
  "user_id": "123456789",
  "reason": "violation",
...
##### iam_user_registered
avion-authから受信するユーザー登録イベント。新規ユーザーのプロフィール・設定・統計レコードの作成をトリガーする。

##### drop.created / drop.deleted
avion-dropから受信する投稿作成・削除イベント。UserStats の `drop_count` カウンターを増減する。

##### reaction.created / reaction.deleted
avion-dropから受信するリアクション作成・削除イベント。UserStats の `reaction_received_count` カウンターを増減する。

##### avion.user.account.deletion_completed
各サービスから受信するユーザー削除カスケード完了報告イベント。avion-user がオーケストレーターとして全サービスの完了を追跡し、すべての削除が完了した時点でユーザー基本レコードを物理削除する。詳細は [ユーザー削除カスケード設計](../common/architecture/user-deletion-cascade.md) を参照。
...
- **フロー 1: ユーザー作成 (Command)**
    ...
    5. CreateUserCommandUseCase: EventPublisherを通じて `avion.user.user.created` イベントを発行
...
- **フロー 3: プロフィール更新 (Command)**
    ...
    7. UpdateProfileCommandUseCase: EventPublisherを通じて `avion.user.profile.updated` イベントを発行
...
- **フロー 4: フォロー (Command)**
    ...
    7. FollowUserCommandUseCase: EventPublisherを通じて `avion.user.follow.created` イベントを発行
...
- **フロー 5: フォロー解除 (Command)**
    ...
    6. UnfollowUserCommandUseCase: EventPublisherを通じて `avion.user.follow.deleted` イベントを発行
...
- **フロー 6: ユーザーブロック (Command)**
    ...
    6. BlockUserCommandUseCase: EventPublisherを通じて `avion.user.block.created` イベントを発行
...
logger.Info("user account created",
    ...
    slog.String("event_type", "avion.user.user.created"),
    slog.String("user_id", userID),
    slog.String("username", username),
...
logger.Info("user profile updated",
    ...
    slog.String("event_type", "avion.user.profile.updated"),
    slog.String("user_id", userID),
    slog.Group("changes",
...
各ドメインオブジェクトとDBスキーマの対応関係は以下の通りです。DDL定義とRedisキャッシュ構造の詳細は [7.3. データモデル](#73-データモデル) を参照してください。

| ドメインオブジェクト | DBテーブル | ID型 |
|---|---|---|
| User Aggregate | users | UUID v7 |
| ProfileField Entity | profile_fields | UUID v7 |
| Follow Aggregate | follows | UUID v7 |
| FollowRequest Entity | follow_requests | UUID v7 |
| Block Aggregate | blocks | UUID v7 |
| Mute Aggregate | mutes | UUID v7 |
| MuteKeyword Entity | mute_keywords | UUID v7 |
| UserSettings Aggregate | user_settings | UUID v7 (users.user_id) |
| UserList Aggregate | user_lists | UUID v7 |
| ListMember Entity | list_members | UUID v7 |
| UserStats Aggregate | user_stats | UUID v7 (users.user_id) |

## 18. Migration Strategy & Release Plan (移行戦略・リリース計画)
...
- User Aggregate、基本CRUD API、PostgreSQLスキーマ作成
- UserRepository実装、基本的なプロフィール管理機能
- ユーザー情報のRedisキャッシュ実装
- リリース基準: ユニットテストカバレッジ85%以上、基本APIのレイテンシp99 < 200ms

#### Phase 2: Social Graph (Week 3-4)
- Follow/Block/Mute Aggregate実装
- フォロー/アンフォロー、ブロック、ミュート機能
- フォローリクエスト機能（承認制アカウント対応）
- キーワードミュート機能
- リリース基準: フォロー関係操作のレイテンシp99 < 300ms、関係整合性テスト合格

#### Phase 3: Settings & Lists (Week 5-6)
- UserSettings Aggregate、UserList Aggregate実装
- 設定管理API、リスト管理API
- プライバシー設定に基づくアクセス制御
- リリース基準: 設定更新のレイテンシp99 < 250ms、プライバシー制御のE2Eテスト合格

#### Phase 4: Events & Integration (Week 7-8)
- NATS JetStreamによるイベント発行・購読実装
- 他サービス（avion-search、avion-timeline、avion-activitypub）との連携
- UserStats Aggregate実装
- リリース基準: イベント配信遅延 < 100ms、サービス間連携のE2Eテスト合格

#### Phase 5: Optimization & GA (Week 9-10)
- パフォーマンス最適化（キャッシュウォーミング、クエリ最適化）
- 負荷テスト（10,000 req/s目標）
...
// TestFollowUseCase_ConcurrentFollowUnfollow は並行フォロー/アンフォロー操作をテストする
func TestFollowUseCase_ConcurrentFollowUnfollow(t *testing.T) {
    tests := []struct {
        name          string
        followerID    string
        followeeID    string
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            got, err := repo.IsFollowing(ctx, tt.followerID, tt.followeeID)
            if err != nil {
                t.Fatalf("IsFollowing failed: %v", err)
            }
            if diff := cmp.Diff(tt.expectedState, got); diff != "" {
                t.Errorf("follow state mismatch (-want +got):\n%s", diff)
            }

            gotFollowerCount, err := repo.GetFollowerCount(ctx, tt.followeeID)
            if err != nil {
                t.Fatalf("GetFollowerCount failed: %v", err)
            }
            gotFollowingCount, err := repo.GetFollowingCount(ctx, tt.followerID)
            if err != nil {
                t.Fatalf("GetFollowingCount failed: %v", err)
            }

            if tt.expectedState {
                if diff := cmp.Diff(int64(1), gotFollowerCount); diff != "" {
                    t.Errorf("follower count mismatch (-want +got):\n%s", diff)
                }
                if diff := cmp.Diff(int64(1), gotFollowingCount); diff != "" {
                    t.Errorf("following count mismatch (-want +got):\n%s", diff)
                }
            } else {
                if diff := cmp.Diff(int64(0), gotFollowerCount); diff != "" {
                    t.Errorf("follower count mismatch (-want +got):\n%s", diff)
                }
                if diff := cmp.Diff(int64(0), gotFollowingCount); diff != "" {
                    t.Errorf("following count mismatch (-want +got):\n%s", diff)
                }
            }
        })
...
// TestRelationshipUseCase_MutualFollowDetection は相互フォロー検出をテストする
func TestRelationshipUseCase_MutualFollowDetection(t *testing.T) {
    tests := []struct {
        name           string
        user1ID        string
        user2ID        string
    ...
    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            ...
            got, err := usecase.GetRelationship(ctx, tt.user1ID, tt.user2ID)
            if err != nil {
                t.Fatalf("GetRelationship failed: %v", err)
            }
            if diff := cmp.Diff(tt.expectedMutual, got.IsMutualFollow); diff != "" {
                t.Errorf("mutual follow mismatch (-want +got):\n%s", diff)
            }
        })
    }
}
...
```go
// TestBlockUseCase_CascadeAcrossServices はブロック操作のサービス間カスケードをテストする
func TestBlockUseCase_CascadeAcrossServices(t *testing.T) {
    tests := []struct {
        name              string
        blockerID         string
        blockedID         string
        ...
        {
            name:      "正常系: ブロック操作がタイムラインと通知サービスにカスケード",
            blockerID: "user1",
            blockedID: "user2",
            expectedEvents: []string{
                "avion.user.block.created",
...
// TestMuteUseCase_DoesNotAffectFollowRelationship はミュートがフォロー関係に影響しないことをテストする
func TestMuteUseCase_DoesNotAffectFollowRelationship(t *testing.T) {
    ctx := context.Background()
    repo := setupTestRepository(t)
...
```go
// TestProfileUseCase_CustomFieldsValidation はカスタムプロフィール項目のバリデーションをテストする
func TestProfileUseCase_CustomFieldsValidation(t *testing.T) {
    tests := []struct {
        name          string
...
// TestProfileUseCase_AvatarImageProcessing はアバター画像の処理パイプラインをテストする
func TestProfileUseCase_AvatarImageProcessing(t *testing.T) {
    tests := []struct {
        name            string
        imageData       []byte
        contentType     string
...
```go
// TestSettingsUseCase_Synchronization は複数デバイス間の設定同期をテストする
func TestSettingsUseCase_Synchronization(t *testing.T) {
    tests := []struct {
        name                string
...
// TestSettingsUseCase_ExportImport は設定のエクスポート・インポート機能をテストする
func TestSettingsUseCase_ExportImport(t *testing.T) {
    ctx := context.Background()
    repo := setupTestRepository(t)
...
```go
// TestGraphTraversalUseCase_LargeScale は大規模ソーシャルグラフでの性能をテストする
func TestGraphTraversalUseCase_LargeScale(t *testing.T) {
    if testing.Short() {
    ...
    tests := []struct {
        ...
        {
            name:             "性能テスト: 1000ユーザーの中規模グラフ",
            userCount:        1000,
            followsPerUser:   50,
            maxTraversalTime: 100 * time.Millisecond,
        },
        {
            name:             "性能テスト: 10000ユーザーの大規模グラフ",
            userCount:        10000,
            followsPerUser:   100,
